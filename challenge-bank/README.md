...
|---|---|---:|---:|
| Math AA HL | 2021 (runs to Nov 2028) | 109 | 83 / 83 (100%) |
| Physics HL | 2025 | 75 | 24 / 24 (100%) |
| Computer Science HL | 2027 (new Theme A/B) | 41 | 25 / 25 (100%) |
| Business Management SL | 2024 | 30 | 34 / 34 (100%) |
| **Total** | | **255** | **166 / 166 (100%)** |
...
All 255 items are difficulty 3–5 (**3 at difficulty 3, 150 at difficulty 4, 102 at difficulty 5**), and
pass `validate.py`
...
Every item carries a `verification.assertions` list — **2840 machine-checked assertions** in total — so
the arithmetic in every answer is re-derived by the validator on each run, not merely asserted by the
author. **45 items are figure-bearing**, with the SVG generated by `tools/make_figures.py` and inlined
into `question.figure` so a page renders identically on `file://` and over HTTP.

**Sourcing is recorded, not claimed.** 60 items are drawn from other syllabuses — 高考, 强基, 竞赛,
...
|---|---|---|
| items with evidence for the label | **91 / 255** | 0 / 172 |
...
points"* — was the last calibration debt, and it is paid: three items now claim difficulty 3, one each in
Maths (`MATH-AHL2.15-201`), CS (`CS-B2.2-201`) and BM (`BM-4.1-201`). The audit prints `calibration OK`
and `--check --strict`, which fails on a debt as well as on a regression, exits 0 for the first time.
Physics still has none, and that is deliberate rather than overlooked: every Physics node whose natural
idea is a single clean lever — orbital energy in D.1, the two-stage Doppler shift in C.5, the end
correction in C.4, the massive-pulley tensions in A.2/A.4, the Carnot comparison in B.4 — already carries
an item that uses it, so a Physics difficulty-3 item needs a new design rather than a slot filled.

**The rubric score cannot produce a difficulty-3 item, and this is worth knowing before trying.**
Measured across all 90 evidenced items: **every one scores 9/9**, including all 65 at difficulty 4 and all
23 at difficulty 5. The score is a documentation gate — are the three evidence statements present, long
...
inside the cap and still below the 62% baseline. The calibration gate is checked against the baseline on
...
largest single share is 15% of evidenced items, so the bank is no longer one trick in different clothes.
