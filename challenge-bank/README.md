...
|---|---|---:|---:|
| Math AA HL | 2021 (runs to Nov 2028) | 81 | 83 / 83 (100%) |
| Physics HL | 2025 | 54 | 24 / 24 (100%) |
| Computer Science HL | 2027 (new Theme A/B) | 28 | 25 / 25 (100%) |
| Business Management SL | 2024 | 21 | 34 / 34 (100%) |
| **Total** | | **184** | **166 / 166 (100%)** |
...
All 184 items are difficulty 4–5 (**100 at difficulty 4, 84 at difficulty 5**), and pass `validate.py`
...
Every item carries a `verification.assertions` list — **1902 machine-checked assertions** in total — so
the arithmetic in every answer is re-derived by the validator on each run, not merely asserted by the
author. **32 items are figure-bearing**, with the SVG generated by `tools/make_figures.py` and inlined
into `question.figure` so a page renders identically on `file://` and over HTTP.

**Sourcing is recorded, not claimed.** 18 items are drawn from other syllabuses — 高考, 强基, 竞赛,
A-Level, Further Maths, AP — and each names `provenance.resource_origin` and `provenance.adaptation`.
...
|---|---|---|
| items with evidence for the label | **20 / 184** | 0 / 172 |
...
rules are still breached (Physics HL puts 61% of its items at difficulty 5 against a 50% limit, and no
...
The Physics share has now moved the right way: it was **62% at the baseline**, rose to **63%** in Batch 13
when two Physics items shipped at difficulty 5, and fell to **61%** in Batch 14 because both new Physics
items were written at difficulty 4 with heavier `Show that` scaffolding. That is the intended mechanism
for paying the debt down — add d4 items rather than relabel earned d5 items — and the rule that the next
Physics item may not claim difficulty 5 until the share reaches 50% remains in force.
