...
All 202 items are difficulty 4–5 (**116 at difficulty 4, 86 at difficulty 5**), and pass `validate.py`
...
Every item carries a `verification.assertions` list — **2088 machine-checked assertions** in total — so
...
|---|---|---|
| items with evidence for the label | **38 / 202** | 0 / 172 |
...
largest single share has fallen from 43% to 18% of evidenced items, so the bank is no longer one trick
