...
|---|---|---:|---:|
| Math AA HL | 2021 (runs to Nov 2028) | 119 | 83 / 83 (100%) |
| Physics HL | 2025 | 84 | 24 / 24 (100%) |
| Computer Science HL | 2027 (new Theme A/B) | 46 | 25 / 25 (100%) |
| Business Management SL | 2024 | 36 | 34 / 34 (100%) |
| **Total** | | **285** | **166 / 166 (100%)** |
...
All 285 items are difficulty 3–5 (**4 at difficulty 3, 164 at difficulty 4, 117 at difficulty 5**), and
pass `validate.py`
...
Every item carries a `verification.assertions` list — **3271 machine-checked assertions** in total — so
the arithmetic in every answer is re-derived by the validator on each run, not merely asserted by the
author. **59 items are figure-bearing (21%)**, with the SVG generated by `tools/make_figures.py` and
inlined into `question.figure` so a page renders identically on `file://` and over HTTP.

**The figures are hand-drawn, and the gate enforces it.** Every one of the 59 is emitted as raw SVG by a
plain-Python string builder in `tools/make_figures.py` — no charting library, no plotting package, no
image model. That is not a style preference: `validate.py` rejects the fingerprints of all of them
(`<canvas`, `plotly`, `matplotlib`, `chart.js`, `highcharts`, `echarts`, `vega`, `bokeh`, `data:image/`),
so an item cannot smuggle in a generated or rasterised picture. Two of the newest figures sample their own
curve from the same expression the markscheme integrates, so the picture and the answer cannot drift apart.
Coverage is tracked as a **ratchet**: `difficulty_audit.py` holds a bank-wide floor
(`FIGURE_COVERAGE_FLOOR`, now **0.20**) that may rise but may never fall, so a later batch cannot add
text-only items and let the share sag back. The per-subject companion (`FIGURE_SUBJECT_TARGET = 0.15`) is
*reported* rather than enforced — Maths sits at 11% and BM at 14%, and both gaps are printed on every audit
run rather than papered over.

**Sourcing is recorded, not claimed.** 88 items are drawn from other syllabuses — 高考, 强基, 竞赛,
...
|---|---|---|
| items with evidence for the label | **113 / 277** | 0 / 172 |
...
Measured across all 113 evidenced items: **every one scores 9/9**, including all 78 at difficulty 4 and
...
difficulty 4 and three at difficulty 5 — landing at **44%**. The calibration gate is checked against
the baseline on
