...
|---|---|---:|---:|
| Math AA HL | 2021 (runs to Nov 2028) | 93 | 83 / 83 (100%) |
| Physics HL | 2025 | 59 | 24 / 24 (100%) |
| Computer Science HL | 2027 (new Theme A/B) | 32 | 25 / 25 (100%) |
| Business Management SL | 2024 | 24 | 34 / 34 (100%) |
| **Total** | | **208** | **166 / 166 (100%)** |
...
All 208 items are difficulty 4–5 (**117 at difficulty 4, 91 at difficulty 5**), and pass `validate.py`
...
Every item carries a `verification.assertions` list — **2174 machine-checked assertions** in total — so
the arithmetic in every answer is re-derived by the validator on each run, not merely asserted by the
author. **35 items are figure-bearing**, with the SVG generated by `tools/make_figures.py` and inlined
into `question.figure` so a page renders identically on `file://` and over HTTP.

**Sourcing is recorded, not claimed.** 36 items are drawn from other syllabuses — 高考, 强基, 竞赛,
...
|---|---|---|
| items with evidence for the label | **44 / 208** | 0 / 172 |
...
rules are still breached (Physics HL puts 56% of its items at difficulty 5 against a 50% limit, and no
...
The Physics share is being paid down the intended way — add d4 items rather than relabel earned d5
items. It stood at **62% at the baseline**, rose to **63%** in Batch 13 when two Physics items shipped
at difficulty 5, fell to **61%** in Batch 14 when both new Physics items were written at difficulty 4
with heavier `Show that` scaffolding, and fell again to **56%** in Batch 15 when all five new Physics
items were designed at difficulty 4 from the outset. The rule that no new Physics item may claim
difficulty 5 until the share reaches 50% remains in force, and Batch 15 is the first batch in which
that rule shaped the design rather than merely constraining the labels.

Batch 15 also closed the last gap in the lever taxonomy: **all 13 lever types are now in use**, and the
largest single share has fallen from 43% to 20% of evidenced items, so the bank is no longer one trick
