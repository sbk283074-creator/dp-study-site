...
All 320 items are difficulty 3–5 (**5 at difficulty 3, 182 at difficulty 4, 133 at difficulty 5**), and
pass `validate.py`
...
Every item carries a `verification.assertions` list — **3656 machine-checked assertions** in total — so
the arithmetic in every answer is re-derived by the validator on each run, not merely asserted by the
author. **94 items are figure-bearing (29%)**, with the SVG inlined into `question.figure` so a page
renders identically on `file://` and over HTTP.

**The figures are hand-drawn, and the gate enforces it.** Every one of the 94 is emitted as raw SVG by a
...
(`FIGURE_COVERAGE_FLOOR`, now **0.29**) that may rise but may never fall, so a later batch cannot add
text-only items and let the share sag back. Batch 26 raised it from 0.25 to 0.27; Batch 27 to 0.28;
Batch 28 raised it again, because all five of its items carry a hand-authored figure, which moved
coverage to 94/320 = 29.4% and let the floor follow it up. That leaves
only **4 non-figure items of headroom** (n = 324 passes at 0.29; n = 325 fails), so a plain-text batch
is still effectively ruled out — the ratchet has reached the point where it forces the issue rather than
merely encouraging it. Batches 27 and 28 were both aimed by reading the per-subject column rather than
the gap line, which never prints because no subject is below target. Batch 28 went further and chose its
five items by **paper structure**: CS P1 Section A carries 56 of that paper's 80 marks, but it held only
12 of the bank's 44 CS P1 items, and five Theme A nodes — A1.4, A2.4, A3.3, A3.4 and A4.4 — had no
Section A item at all. Batch 28 fills exactly those five, taking Section A to **17 of 49**, the number of
Theme A nodes represented in Section A from 7 to 12, and CS's figure share from 43% to **48%**. The
per-subject companion
(`FIGURE_SUBJECT_TARGET = 0.15`) is *reported* rather than enforced, and no subject prints a gap:
**CS 48%, Physics 37%, BM 21%, Maths 18%**.

**Sourcing is recorded, not claimed.** 114 items are drawn from other syllabuses — 高考, 强基, 竞赛,
...
|---|---|---|
| items with evidence for the label | **235 / 320** | 0 / 172 |
...
trying.** Measured across all 235 evidenced items: **215 score 9/9**, 19 score 8/9 and one scores 7/9 —
...
largest single share is **14%** of evidenced items (`non_governing_variable`, 34 of 235), so the bank is
no longer one trick in different clothes. That share barely moved while the evidenced set grew from
**38 items to 235** — it was 18% of those 38, measured at the Batch 15 commit — which is the useful
...
more than a point. Batch 28 added no Physics items, so the Physics share above is unchanged at 44%.
