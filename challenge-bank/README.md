...
|---|---|---:|---:|
| Math AA HL | 2021 (runs to Nov 2028) | 108 | 83 / 83 (100%) |
| Physics HL | 2025 | 75 | 24 / 24 (100%) |
| Computer Science HL | 2027 (new Theme A/B) | 40 | 25 / 25 (100%) |
| Business Management SL | 2024 | 29 | 34 / 34 (100%) |
| **Total** | | **252** | **166 / 166 (100%)** |
...
All 252 items are difficulty 4–5 (**150 at difficulty 4, 102 at difficulty 5**), and pass `validate.py`
...
Every item carries a `verification.assertions` list — **2788 machine-checked assertions** in total — so
the arithmetic in every answer is re-derived by the validator on each run, not merely asserted by the
author. **43 items are figure-bearing**, with the SVG generated by `tools/make_figures.py` and inlined
into `question.figure` so a page renders identically on `file://` and over HTTP.

**Sourcing is recorded, not claimed.** 59 items are drawn from other syllabuses — 高考, 强基, 竞赛,
...
|---|---|---|
| items with evidence for the label | **88 / 252** | 0 / 172 |
