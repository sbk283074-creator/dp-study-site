...
def series_solution_ode():
    """Maclaurin partial sums of y' = y^2 - x, y(0) = 1, against the solution.

    The reference curve is generated by the same Runge-Kutta integration the
    answer quotes, so the partial sums can be seen to approach the curve the
    arithmetic produces rather than a drawn-by-eye approximation of it.
    """
    W, H = 470, 268
    x0, x1 = 62, 430
    top, base = 34, 214
    ...
    def runge(h=0.0005):
        px, py, pts = 0.0, 1.0, [(0.0, 1.0)]
        while px < xe - 1e-12:
            k1 = py * py - px
            k2 = (py + h * k1 / 2) ** 2 - (px + h / 2)
            k3 = (py + h * k2 / 2) ** 2 - (px + h / 2)
            k4 = (py + h * k3) ** 2 - (px + h)
            py += h * (k1 + 2 * k2 + 2 * k3 + k4) / 6.0
            px += h
            if round(px / 0.005) == px / 0.005:
                pts.append((px, py))
        return pts

    A = [1.0, 1.0, 1 / 2, 2 / 3, 7 / 12, 11 / 20, 22 / 45, 559 / 1260, 447 / 1120]

    def exact(x):
        return 1.0 / (1.0302 - x)

    def X(v):
        return x0 + (v / xe) * (x1 - x0)

    def Y(v):
        return base - (v - ylo) / (yhi - ylo) * (base - top)

    def partial(x, n):
        return sum(A[k] * x ** k for k in range(n + 1))
    ...
    for v in (1.0, 1.2, 1.4, 1.6):
        body.append(_l(x0, Y(v), x1, Y(v), PANEL2, 1.0))
        body.append(_t(x0 - 8, Y(v) + 4, f"{v:.1f}", 10.5, MUTED, anchor="end"))
    for v in (0.1, 0.2, 0.3, 0.4):
        body.append(_l(X(v), base, X(v), base - 5, MUTED, 1.0))
        body.append(_t(X(v), base + 17, f"{v:.1f}", 10.5, MUTED))
    body.append(_l(x0, base, x1 + 10, base, INK, 1.6))
    body.append(_l(x0, base, x0, top - 4, INK, 1.6))
    body.append(_t(x1 + 16, base + 4, "x", 12, INK, anchor="start"))
    body.append(_t(x0 - 30, top - 6, "y", 12, INK, anchor="start"))

    body.append(_p(path(lambda x: partial(x, 1), 0.004), stroke=HOT, w=1.5, dash="5 3"))
    body.append(_p(path(lambda x: partial(x, 5), 0.004), stroke=WARN, w=1.5, dash="2 3"))
    body.append(_p(path(exact, 0.004), stroke=INK, w=1.8))
    body.append(_p(path(lambda x: partial(x, 7), 0.004), stroke=GOOD, w=1.5, dash="7 3"))

    body.append(_t(x0 + 6, top + 4, "y = solution of y' = y² − x, y(0) = 1", 11, INK, anchor="start"))
    body.append(_l(x0 + 8, top + 22, x0 + 34, top + 22, HOT, 1.5, dash="5 3"))
    body.append(_t(x0 + 40, top + 26, "P₁(x) = 1 + x", 11, MUTED, anchor="start"))
    body.append(_l(x0 + 8, top + 40, x0 + 34, top + 40, WARN, 1.5, dash="2 3"))
    body.append(_t(x0 + 40, top + 44, "P₅(x), the first six terms", 11, MUTED, anchor="start"))
    body.append(_l(x0 + 8, top + 58, x0 + 34, top + 58, GOOD, 1.5, dash="7 3"))
    body.append(_t(x0 + 40, top + 62, "P₇(x), the first eight terms", 11, MUTED, anchor="start"))
    return _svg(W, H, "Partial sums of the Maclaurin series of the solution of y dash equals y squared "
                      "minus x with y at zero equal to one, plotted against the numerical solution on "
                      "the interval from zero to zero point four five. The sums approach the curve and "
                      "still fall short of it at x equal to zero point four five.", "".join(body))


def compile_interpret_cost():
    """Total time against number of executions: interpret every time vs compile once.

    The two lines are drawn from the same cost model the question states, so the
    crossing point sits exactly at the integer the answer computes.
    """
    W, H = 470, 276
    x0, x1 = 66, 424
    top, base = 36, 214
    ...
    def X(n):
        return x0 + (n / ne) * (x1 - x0)

    def Y(t):
        return base - min(t, yhi) / yhi * (base - top)

    def line(slope, intercept):
        return _l(X(0), Y(intercept), X(ne), Y(slope * ne + intercept), ACCENT if intercept else HOT, 1.8)

    body = [_arrow_defs()]
    for t in (0, 500, 1000, 1500):
        body.append(_l(x0, Y(t), x1, Y(t), PANEL2, 1.0))
        body.append(_t(x0 - 8, Y(t) + 4, str(t), 10.5, MUTED, anchor="end"))
    for n in (0, 20, 40, 48, 60, 80):
        body.append(_l(X(n), base, X(n), base - 5, MUTED, 1.0))
        body.append(_t(X(n), base + 17, str(n), 10.5, MUTED))
    body.append(_l(x0, base, x1 + 10, base, INK, 1.6))
    body.append(_l(x0, base, x0, top - 4, INK, 1.6))
    body.append(_t(x1 - 4, base + 32, "executions of the same script", 11.5, INK, anchor="end"))
    body.append(_tr(x0 - 42, top + 90, "total time / ms", -90, 11.5, INK))

    body.append(line(18.0, 0.0))
    body.append(line(3.0, 720.0))

    body.append(_l(X(48), base, X(48), Y(864), MUTED, 1.2, dash="4 3"))
    body.append(_c(X(48), Y(864), 4.2, fill="#ffffff", stroke=MUTED, w=1.6))
    body.append(_t(X(48) + 10, Y(864) - 10, "equal at 48 executions, 864 ms each", 11, ACCENT, anchor="start"))
    body.append(_t(x0 + 8, top + 6, "interpreted: 18 ms per execution", 11, HOT, anchor="start"))
    body.append(_t(x1 - 6, top + 6, "compiled: 720 ms once, then 3 ms per execution", 11, ACCENT, anchor="end"))
    body.append(_t(x0 + 8, base - 8, "the compiler is slower until the script has been run 48 times", 11.5, MUTED, anchor="start"))
    return _svg(W, H, "Total running time against the number of executions of the same script. The "
                      "interpreted line rises from the origin at eighteen milliseconds per execution; "
                      "the compiled line starts at seven hundred and twenty milliseconds and rises at "
                      "three milliseconds per execution. They cross at forty eight executions, where "
