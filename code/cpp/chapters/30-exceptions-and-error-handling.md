...
Chapter 21's `handle` owned three resources and had five ways out of it. Chapter 24's RAII made the
...
This is also where `noexcept` earns its keep elsewhere. Chapter 25 marked the move constructor `noexcept`
...
Chapter 21's `handle` had three resources and five exits, and the only reason it was correct was that
...
`catch (std::exception e)` looks harmless and compiles. It is the Chapter 27 slicing problem applied to
...
dispatches to the override. It is the Chapter 27 rule applied to handlers: **a polymorphic type is never
caught by value.**
