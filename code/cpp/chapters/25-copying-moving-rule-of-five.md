...
Chapter 24 ended with a warning: the `Buffer` class it taught would double-free if you ever copied it,
...
- The rule of zero: put ownership in a member that already handles it (`std::string`, `std::vector`, and
  in Chapter 26 `std::unique_ptr`) and write no destructor, no copy and no move at all.
