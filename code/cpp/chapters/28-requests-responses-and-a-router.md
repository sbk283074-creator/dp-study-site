...
Chapter 27's server reads one line and compares it:
...
and the `Router` that chooses between them. The socket layer from Chapter 27 is unchanged. What
...
problem as the `close(conn)` Chapter 27 opened with. The fix is to make the wrong case *impossible* by
...
distinction Chapter 25 drew, and here it decides whether `if (headers.get("content-length"))` means
what you think.
...
`std::optional<Request>` rather than a thrown exception, and the choice is the one Chapter 26 drew: a
...
matters more than it looks: Chapter 27's lesson was that a security check must run on the value you
...
without mutating the response the caller still holds. Chapter 21's copy semantics, used for exactly
what they are for.

`Content-Length` comes from `body.size()`, never from a parameter. That was Chapter 27's lesson and it
...
entries is nothing next to reading a file or opening a socket. The alternative from Chapter 23 is an
...
A `string_view` is a **non-owning** view, and that is the entire hazard. It is Chapter 19's reference
...
- **Store a `std::string`.** A member that holds a view into something else is a member whose validity
  depends on something the class does not control — the same trap Chapter 22 described for raw
...
and test without a socket in sight. Chapter 27's server had the same property and used it for the
...
The bug is in Chapter 27's socket code, and it has been there all along:
...
/* Chapter 27's read_until: stop as soon as the blank line arrives. */
...
the layer above a complete message.** Chapter 27's `read_until` stopped at the first boundary it found
...
Do not catch `...`. Chapter 26's warning applies: a bare catch swallows the bugs you most need to see,
