...
Look at `handle` from Chapter 17 again. It took a request, produced a response, and closed three
...
Two things changed from the C structs in Chapter 9. `int x = 0;` is a **default member initialiser** —
...
named `lo` never collide. And the accessors are marked `const` — as Chapter 19 established, that is what
...
methods. We use the split in Chapter 22 when a class grows past one screen. Note that the `const` on
...
distinction Chapter 12 taught. A static method has no `this`, so it cannot touch non-static members;
...
The `static_cast<char *>` is the C++ requirement from Chapter 18: `std::malloc` returns `void *` and
...
in the standard library either forbids copying or implements it properly. Chapter 21 is entirely about
...
- An RAII class acquires in the constructor and releases in the destructor, which makes cleanup
  automatic on every exit path — including early returns and (Chapter 26) exceptions.
...
move it. `pop` takes its result through a reference parameter (Chapter 19) so it can return `bool`
