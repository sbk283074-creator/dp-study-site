...
is what has been paid for. Chapter 24 replaces this struct with `std::vector<int>` and deletes the file,
...
Chapter 9 said a struct assignment copies every byte. For a struct containing an *array* that is exactly
...
would be the `DEADLYSIGNAL` crash from Chapter 8. Duplicating onto the heap is what makes the uppercase
