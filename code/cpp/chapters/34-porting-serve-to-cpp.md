...
Chapter 21's server works. Five `.c` files, a Makefile, a self-test, and a hardening checklist it
...
Here is the shape of `serve_one` from Chapter 21, reduced to what matters:
...
and large enough that the cause is nowhere near the symptom. Chapter 21's checklist asked "does every
...
`0`. That `0` is a value the caller has to check, which is why Chapter 21's `serve_one` carries this:
...
is the failure Chapter 21's checklist called "a truncated body with a `Content-Length` for the full
...
`std::unique_ptr` from Chapter 26 is the tool for a resource you acquire once and release in one place,
...
Everything above, assembled into the same four layers Chapter 21 had. `path` has become `files`, and
...
std::string Server::handle(const std::string &raw) const {
    /* Chapter 35 replaces this with a real request model; for now the request
...
/* Print a response with CR LF made visible, the way chapter 21 did. */
...
Put that next to Chapter 21's transcript and the comparison is the point of the whole chapter. The
...
The Makefile is Chapter 21's, with `CC` changed to `CXX` and `-std=c17` to `-std=c++17`. Everything
...
to fetch will get the answer wrong. Chapter 21 left this as an exercise. Here it is one line:
...
destruction order. That is the same ordering rule Chapter 24 showed you with `-Wreorder-ctor`, seen
...
The `static` definition outside the class is required, and it is the same rule Chapter 24 covered:
...
- **Too long** — the request exceeded the cap. This is `414 URI Too Long`, which is a real status code
  for exactly this situation and which Chapter 21 used. The version that returns a truncated string
...
2. Make `serve_forever` return when a signal arrives — install a handler for `SIGINT` that sets a
   ...
   the standard shape and it is what Chapter 41 does properly, with `std::atomic` instead.
3. Count in a way that survives the process, which is Chapter 38's problem when the service has more
   than one process.
