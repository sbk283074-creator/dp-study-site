...
Every function in Part III had a cleanup path. `handle` in Chapter 17 had to free the body, close the
...
that makes overloading work in Chapter 19. What you see above is the linker demangling the name back
...
its array of strings. The `&` in `const std::string &name` is a **reference** — Chapter 19's subject —
...
releases, and the language guarantees the second one runs. Chapter 17's `handle` had to free the body,
...
`new` at all — Chapter 22's `std::unique_ptr` is the standard answer, and it is the reason the rest of
...
exactly like Chapter 17's `handle`: it opens a file, allocates a buffer, and has four `return`
...
- C++ does not remove memory errors. It gives you the tools to avoid typing `new` — Chapter 22 is
...
Two constructors with different parameter lists is **overloading**, which Chapter 19 covers properly.
