...
Chapter 21 ended with the most useful piece of advice in this part of the book: put ownership in a
...
A raw pointer `T *` says nothing about ownership. In Chapter 20's `Buffer` the member `char *data_` was
...
free from Chapter 21 — two owners, one allocation. Instead, the only way to hand the object to someone
...
moved-from convention from Chapter 21, and it is why a `unique_ptr` in a boolean context tells you
...
reason this cannot be the `new[]`/`delete` mismatch from Chapter 18's pitfall.
...
destructor — which is the bug Chapter 20 was about, only now spread across two classes. The C++ version
...
The review question from Chapter 21 becomes answerable at a glance: every member is either a value, a
...
Chapter 20's `File` class collapses into one line at the point of use. `log.get()` is how you reach the
raw `std::FILE *` for `std::fputs`, which needs one.

The lambda is a preview of Chapter 25. For now, read `[](std::FILE *f) { std::fclose(f); }` as "an
...
because neither knows the other exists. This is the Chapter 21 double free wearing a safety feature's
...
at the end of `main`, which is why the program never calls it. This is the whole of Chapter 20's `File`
