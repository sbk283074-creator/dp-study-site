...
Every function written so far has been one function for one type. Chapter 27's `Exporter` had three
...
Chapter 29 can be a tour of containers and algorithms rather than a list of hundreds of functions.
...
per-object overhead. Unlike the virtual functions in Chapter 27, a template costs nothing at run time —
...
template cannot be split into a header and a `.cpp` the way Chapter 14's functions were:
...
is the same class of failure as Chapter 14's missing definition, arriving through a new route.
...
Chapter 10 taught that an array passed to a function becomes a pointer to its first element, losing its
