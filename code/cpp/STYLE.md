...
| ` ```cpp run-abort ` | A program that **must die**, with a non-zero exit. | Compiles with `-Wall -Wextra -Werror`, then requires a non-zero exit and the `text` fence to appear in **stderr**. For failures the C++ *runtime* catches rather than the sanitizer: a throwing destructor during unwinding, an uncaught exception, an assertion. |
