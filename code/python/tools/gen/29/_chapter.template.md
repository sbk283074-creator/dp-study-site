...
## Counted: what each probe touches, and what a grace period cuts

Two claims decide whether a deploy is survivable: which checks each health probe
runs, and how long the process is given to finish what it started.

### Two probes, two questions

Six checks split across a liveness probe and a readiness probe. The count is of
external dependencies each probe touches, because that is the number that
decides what happens during an outage.

<!--BLOCK:readiness_checks-->

The two probes answer different questions and must not share their checks.
Liveness asks whether the process can still run, and the answer decides one
thing: restart it. Readiness asks whether it can serve a request right now, and
the answer decides a different thing: stop sending it traffic and leave it
running.

The dependency counts are the numbers to look at. The liveness probe checks zero
dependencies, so a cache that is down fails readiness and passes liveness — the
instance leaves the load balancer and is not restarted. If liveness checked the
database instead, a database outage would fail the probe on every healthy
process at the same moment and the orchestrator would restart the whole fleet
into the same outage. That is how a two-minute blip becomes an afternoon.

The other rule is about cost. A readiness check runs every few seconds for the
life of the service, so it has to be cheap and it must not be the thing that
fails first. Opening a database connection per call is a load generator wearing
a health check's name; asking the pool for a connection and returning it is a
check.

### The grace period is set by the slowest request

Six requests in flight when the shutdown signal arrives, and a grace period of
ten ticks. The count is of requests that finish inside it.

<!--BLOCK:shutdown_drain-->

Ten ticks is enough for four of the six, and the two it cuts are the report and
the checkout. That is the shape of the problem: the expensive requests are the
ones a customer noticed, and they are exactly the ones a short grace period
kills.

So the grace period is set by the slowest endpoint and not by the average. Ten
ticks looks generous against a mean of eight and a half and it is not enough —
the number that matters is the twenty-five of the report. A deploy that kills a
checkout mid-write is not a fast deploy, it is a data problem with a timestamp.

There is a second grace period and it is the one people miss. The orchestrator
has its own timeout before it sends SIGKILL and the application has its own
drain timeout. If the orchestrator's is shorter, the process is killed while it
is still draining and the application's setting never applies, which is the same
as having none. The two numbers have to be set together, with the application's
strictly smaller, and that order is what makes the drain real.
