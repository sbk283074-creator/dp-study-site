...
## Counted: text that becomes markup, and JSON that ends an element

Two claims in this chapter are the ones that decide whether a page can be rewritten by its own data:
that an interpolated value is still text, and that JSON can be embedded in HTML unchanged.

### A value put somewhere that parses

Seven strings a user can type, placed into a paragraph and into an attribute, each time with and
without `html.escape`. The count is of values that add markup to the page.

<!--BLOCK:html_escaping-->

The two columns on the right are the ones that matter, and they are the ones people forget. Escaping
for a text position is the habit everybody has; escaping for an attribute is the same function with
`quote=True`, and it is the difference between a value inside the quotes and a value that closes the
quotes and opens an attribute of its own.

The counts are small and the consequence is not. Two values here add an element, which is how a page
gets rewritten; two add an attribute, which is how a script gets attached to an element that was not
going to have one. Both are the same mistake — text was put somewhere that parses — and both are
removed by the same function.

So the rule is about the position, not the value. There is no such thing as a safe string to
interpolate: a string is safe for one context and dangerous in the next, and the same name is safe in
a paragraph and not in an attribute. Escape at the point of use, with the function that knows the
context, and do not assemble HTML by concatenation when a templating engine will do the escaping for
you.

### JSON is not JavaScript

The second claim is the one that connects the backend to the frontend, and it is where a serialiser's
correctness stops being enough.

Five JSON payloads embedded into a script element two ways: raw, and with the three characters that
can end the element replaced. The count is of payloads that survive the trip through the parser.

<!--BLOCK:json_in_script-->

The leak row is the trap: three of the five payloads end the script element early when they are
embedded exactly as `json.dumps` produces them, and the count does not change for the capitalised
form or the one with a space. All three are accepted, because an HTML tag name is case-insensitive
and the close of an element may be followed by whitespace — so a check that looks for the one exact
spelling of the tag finds none of them.

That is not a bug in `json.dumps`. JSON is a data format and `<` is an ordinary character in it, so
all three of the characters that matter to HTML come through untouched — which is correct for a file
and wrong for a page. The two formats agree on almost everything and disagree on exactly the
characters that decide how the page is parsed.

So JSON that is going into HTML is a different encoding problem from JSON that is going into a file.
Escape `<`, `>`, and `&` after serialising — the `\u003c` form, so the result is still valid JSON —
and the payload is data again. Better still, do not put data in a script element at all: send it as a
separate request, or read it from a data attribute that the HTML escaper already handled.
