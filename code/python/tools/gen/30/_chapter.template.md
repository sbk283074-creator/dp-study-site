...
## Counted: what a lapse costs, and what a limit carries

Two claims decide whether the study app is honest with its user: what forgetting
a card actually costs, and whether a session limit is a delay or a deletion.

### The later lapse is the expensive one

One card, a target interval of a year, and three histories: no lapse, a lapse at
the third review, and a lapse at the sixth. The count is of reviews needed to
reach the target.

<!--BLOCK:spaced_repetition-->

Seven reviews take one card past a year, and the table of intervals is the
product: the schedule is a handful of multiplications, and nothing about it
needs a database or a service. That is the point of putting the scheduler in a
pure core — it can be tested against a list of answers like the one above, with
no fixtures and no clock.

The last two rows are the surprising part, and they are why this is worth
counting rather than reasoning about. A lapse at review 3 costs four extra
reviews to get back to a year. A lapse at review 6 costs seven. The later
failure is the more expensive one, because by then the interval has grown to a
hundred and twenty-five days and the reset throws all of it away.

So the shape of the curve is worth knowing before you tune the constants. A
lapse does not cost a fixed amount; it costs what the interval had grown to,
which means the cards that hurt most when you forget them are the ones you had
learned best. That is either the feature you wanted or the one you have to
design around, and you cannot tell which without the numbers.

### A limit delays, a filter drops

Twenty cards, twelve of them due on the first day, and a session limit of eight.
The count is of cards shown and of cards never shown at all.

<!--BLOCK:session_limit-->

Every card is shown eventually. The limit delays the session rather than dropping
the card, and that is the whole difference between a limit and a filter. Twelve
due on the first day and a limit of eight leaves four waiting, so a card that
came due today is shown tomorrow and the backlog is carried rather than
discarded.

The number that matters is the third one, because it decides what the user is
told. A dashboard that says twelve cards due is describing the queue; one that
says eight cards today, four carried over is describing the session. Only the
second is a promise the app can keep, and the first is how a study app teaches
people to stop opening it.

And which eight are chosen is the real design decision, not the limit. Showing
the oldest first clears the backlog, which is what this table does. Showing the
newest first starves the oldest cards permanently — they stay at the front of
the queue while every new card goes ahead of them, and the backlog never clears.
The limit is arithmetic; the order is the product.
