...
## Counted: the orderings that lose an update, and the await that shares a thread

Two claims in this chapter are the ones people take on faith: that a race is a matter of timing, and
that `async def` means concurrency.

### A race is a count, not a timing

Two workers each read a counter, add to it, and write it back. Every interleaving of their six
operations is enumerated. The count is of interleavings that lose an update, and no clock is involved
anywhere.

<!--BLOCK:interleaving_loss-->

Eighteen of the twenty possible orderings lose an update. No clock, no scheduler, no thread — the
count is a property of the code. Two workers read the same value, each adds one to its own copy, and
each writes it back. The second write is not wrong; it is overwriting an answer that was correct at
the moment it was read.

That is why "it passes on my machine" is not evidence about a race. Whether the bad ordering happens
is decided by the machine. How many orderings are bad is decided by the code, and it is decided
before the program runs — which is the number a test can be built on.

The last row is the fix, and it is not a lock. If the read, the add and the write cannot be
separated, there are no interleavings left to lose an update in: the work is one step, and every
ordering of one step is the same ordering. When the operation cannot be made atomic, put a lock
around it. When it can, the lock was never the point.

### One thread, and the await is what shares it

The second claim is about `asyncio`, which is described as concurrency and is really a way of taking
turns.

Four coroutines on one thread, each logging a start and an end, run twice: once with nothing between
the two events and once with a single `await`. The count is of coroutines that reached their end
before any other one had started.

<!--BLOCK:cooperative_switching-->

With nothing between the two events, each coroutine ran its whole body before the next one began:
four starts, four ends, and at no point was more than one of them part-way through. Nothing was
concurrent; the loop simply took them one at a time, and the word `async` bought nothing at all.

With a single `await asyncio.sleep(0)` in the middle, all four reached their start before any of them
reached an end. That is what cooperative switching means: one thread, and the only moment another
coroutine can run is the moment this one hands control back.

Which makes the blocking call the whole problem. `time.sleep`, a synchronous database driver, a
blocking HTTP client — none of them contain an `await`, so none of them hand control back. Put one
inside a coroutine and it behaves exactly like the first column: every other coroutine waits, because
there is one thread and that thread is busy. A coroutine that never awaits is not concurrent code; it
is an ordinary function written with `async def`.
