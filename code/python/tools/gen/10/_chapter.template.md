...
## Counted: what runs once, and what a star import hands over

Two claims in this chapter are worth counting rather than asserting: that a module body runs once
however many times you import it, and that a star import hands over more names than you wrote.

### The body runs once per process

A module written to a temporary directory, imported a hundred times, with the body counting its own
executions. The count is of body executions, of `sys.modules` entries, and of reloads that handed
back the same object.

<!--BLOCK:import_runs_once-->

The first two rows are the whole import cache. A hundred import statements, and the body ran once,
because the second import and every one after it found the object already in `sys.modules`. That is
why the work at the top of a module — reading a file, building a table, opening a connection —
happens once per process rather than once per import line.

The last three rows are the part people get backwards. Reloading runs the body again, five reloads
and six executions in total, but it reuses the module object rather than replacing it, five times
out of five. What changes is the namespace inside that object: the value it holds moved from 41 to
46. A name that was bound earlier by `from counter_module import VALUE` still holds 41, because the
from-import copied the value at the moment it ran. That is the stale name a reload does not repair,
and it is why reloading is not a fix for a module that changed under a running program.

### What a star import hands over

The second claim is about `from module import *`, a statement people write to save typing and then
cannot explain.

Two modules with identical contents, one of which declares `__all__`. Both hold the same five names.
The count is of names delivered, and of names delivered that the module never meant to export.

<!--BLOCK:star_import_names-->

The plain module holds five names and hands over three. The two it withholds are the ones a leading
underscore marks as private, and this is the only place in the language where that convention is
enforced rather than merely respected.

The name to look at is `os`. It is in the plain module's namespace because the module imported it,
and the star import gives it to whoever wrote the import line. So a module that imports `os` for its
own use silently gives `os` to every file that star-imports it, and a name in the importing file
that meant something else is quietly replaced. `__all__` is not a documentation feature: it is the
list of names a star import is allowed to deliver, and without it the answer is "every name that
does not start with an underscore", which includes every module the module imports.
