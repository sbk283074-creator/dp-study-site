...
## Counted: the input a frame can see, and the axis a wall does not block

Two claims decide whether the game feels right: that a press is read as an edge
rather than a level, and that a wall stops the axis it is in and not the player.

### A tap between two frames

Twelve frames of input, one key held for six of them, and five taps placed in
the gaps between the frames. The count is of taps a per-frame poll can see.

<!--BLOCK:input_semantics-->

The first table is the whole difference between the two functions. The key is
down for six frames, `held` reports true on all six, and `pressed` reports true
once. Both are correct and they answer different questions. Anything that should
happen once per press — a jump, a shot, a menu selection — has to read the edge;
reading the level turns one jump into six.

The second table is the part no care in the reading can fix, because it is a
limit of polling rather than a bug in the code. Of the five taps placed in the
gaps between frames, a per-frame poll sees **one**. A tap that begins and ends
between two samples is invisible — the key was never down when a frame was
sampled — and the player presses a button the game never hears.

The last row is the worse case, and it is the one that looks like it works. When
the key is still down at the sample the poll does see the taps, but as one: two
presses produce one jump. That is why a combo system cannot be built on a
per-frame poll however high the frame rate goes, and why the event queue exists.

### Sliding is the free axis

A player stepping diagonally into a wall, resolved two ways. The count is of
steps taken and of the axis the wall actually blocks.

<!--BLOCK:axis_separated-->

Both rows are the same input and they are not the same game. Moving both axes at
once refuses the whole step whenever either one is blocked, so four of the six
steps are thrown away and the player ends at (7, 7) — pressed into the wall with
the free axis unused. Moving one axis at a time refuses only the blocked axis,
so the player keeps the free one and ends at (8, 11).

That is the entire reason platformers resolve axes separately, and it is worth
seeing as a decision rather than a detail. Sliding is not a feature that was
added; it is what happens when the blocked axis is discarded instead of the
whole step. Note which axis is blocked: the wall is a column, and the count is
three steps with x blocked and zero with y blocked — the vertical movement was
never obstructed at all.

The price is the case the table cannot show. A player moving diagonally can pass
through a corner where two walls meet, because neither axis is blocked on its
own. Every engine that resolves per axis has to decide what to do about that:
test the destination square as well, or accept corner-cutting as a movement
technique. Both are defensible; not noticing it is not.
