...
## Counted: what a file is, and what a reader can see

Two claims in this chapter are the kind that sound like advice until you count them: that text and
bytes are different lengths, and that a half-written file is a state a reader can actually observe.

### Characters are not bytes

Six strings, each counted as characters and as UTF-8 bytes, with the same strings offered to latin-1
as well.

<!--BLOCK:encoding_roundtrip-->

The first row is the only one where the two counts agree, and it is the row everybody tests with.
`café` is four characters and five bytes. The CJK string is three characters and nine bytes. The
emoji is one character and four.

The last row is the one to sit with: two code points that a reader sees as a single character,
because the second is a mark that attaches to the first. That is why counting characters is not the
same as counting what a person sees, and why a length limit measured in code points is a different
limit from one measured in what fits on a screen.

The latin-1 column is the second half of the lesson. It round-trips two of the six strings and fails
outright on the other four — not by corrupting them, but by refusing to encode them at all, which is
the better failure. UTF-8 round-trips all six. The reason UTF-8 is the default is not that it is
small; it is that it cannot fail on text a program might legitimately hold.

### The file a reader can see during a write

The other claim worth counting is about `with open(path, "w")`. It is the idiom this chapter
teaches, and it is the wrong tool for writing a file that something else reads.

Three chunks written to one path, two ways, with the file inspected after every chunk. The count is
of observations that are neither the old contents nor the new ones.

<!--BLOCK:atomic_write-->

The first design leaves a file that is not any version of the data on two of the four observations.
A reader arriving in the middle gets a prefix: a settings file that parses up to where it stops, a
document with half a record in it.

The second design shows the old file until the rename and the new file after it, and the rename is a
single operation the filesystem performs atomically. The torn count is zero because there is no
moment in between.

That is what atomic means here, and it is worth stating precisely: not that the write cannot fail,
but that a failure leaves a whole file behind rather than half of one. The cost is one extra file
and one `os.replace`, and it is the difference between a crash that loses the last save and a crash
that corrupts every save after it.
