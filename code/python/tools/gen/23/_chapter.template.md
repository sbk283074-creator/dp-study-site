...
## Counted: the migration that runs once, and the setting that has four answers

Two claims in this chapter are what make a project resumable and reproducible: that a migration is
applied exactly once, and that configuration has one winner per key.

### A migration is a change plus a record

Three schema changes applied by a runner that remembers what it has done. The count is of migrations
run, and of the work a second run does when there is nothing left to do.

<!--BLOCK:migrations-->

The second and third rows are the property that matters. Running the runner twice is the same as
running it once, because each migration checks the record before it does anything. That is what makes
it safe to call at startup, in a deploy script, and again by hand when somebody is not sure whether
it ran — which is the situation that actually happens.

The column count is the other half. Three migrations produced four columns, because a migration adds
to the schema rather than replacing it. The schema in production is the sum of every migration that
has ever run, and the only description of it that is true is the list of files plus the record of
which ones ran.

Which is why a migration that has already run must never be edited. The record says it is done, so
the change never executes again, and the file on disk stops describing the database it created. Add a
fourth migration instead: a wrong migration that has run is history, and history is corrected by what
comes after it.

### Four places a setting can come from

The second claim is about configuration, and it is the part of a service that is hardest to read from
the code.

One setting table with defaults, a config file, the environment and a command-line flag all naming
values. The count is of keys defined in more than one place, which is where the surprises live.

<!--BLOCK:config_precedence-->

The order is the whole design, and it is fixed: default, then file, then environment, then flag. Each
source that names a key replaces the one before it, so the thing typed on this command line always
wins, which is what makes a flag useful for the one run that is different.

The `set in` column is where it goes wrong. Three of these four keys are named in more than one
place, and the busiest is named in all four. A key that appears four times has four candidate answers,
and nothing about the key itself says which one won. You have to know the precedence order — and the
precedence order is not written in the config file the reader is looking at.

So make the resolved configuration printable, and print it at startup in the log. The question "which
value is it actually using" should be answerable from the log of the run, not from reading four files
and knowing the precedence order by heart. And keep the number of sources small: every place a
setting can come from is a place a bug can hide.
