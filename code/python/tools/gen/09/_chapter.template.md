...
## Counted: how wide is your except clause

`except Exception` is the clause most people write and the one this chapter warns about. The warning
is easier to act on as a number.

### What each handler actually catches

Three application errors, four built-in errors, and the two exceptions that are not errors at all —
nine in total. Each handler is asked how many of them it catches.

<!--BLOCK:except_breadth-->

`except NotFound` catches one of the nine and `except AppError` catches three: the three errors this
program defines. They are the two handlers you want, because they name the failures the caller is
prepared to handle, and everything else keeps travelling up the stack to somebody who can.

`except Exception` catches seven. The two it misses are `KeyboardInterrupt` and `SystemExit`, and
neither is a failure of the program — one is the operator asking it to stop and the other is the
interpreter unwinding. That is why seven is the right number rather than a compromise: the broadest
clause a library should write is still narrower than everything.

`except BaseException` catches all nine, which is the count that explains the warning. A handler that
swallows a `KeyboardInterrupt` is a program you cannot stop with Ctrl-C, and it does not swallow one
kind of mistake — it swallows every kind at once, including the two that were never errors.

### Retry, counted in attempts

The other place where the exception type decides something is a retry loop. Six failure patterns
against a budget of three attempts, under two policies.

<!--BLOCK:retry_shapes-->

The two policies agree on five of the six patterns and differ only on the permanent failure, where
retrying everything costs two extra attempts and buys nothing. That is the design of a retry policy
in one line: retry the errors whose next attempt can differ, and give up on the ones whose next
attempt cannot.

The budget is the other half, and the table shows why the type matters as much as the count. Three
patterns exhaust the budget, and in the output they are indistinguishable — a service that was busy
three times and a service that does not exist both produce the same give-up. The exception type is
the only thing that tells them apart, which is why the type is what a handler should name.
