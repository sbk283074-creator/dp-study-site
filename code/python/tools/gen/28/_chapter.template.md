...
## Counted: placeholders that survive, and a handshake that proves itself

Two claims connect this chapter's front end to its back end: that a template
tells you when a value is missing, and that a WebSocket's first response is
derived rather than echoed.

### A page served with a placeholder still in it

One page, five placeholders, three values supplied, rendered three ways. The
count is of renderers that finish and hand the reader a page with a placeholder
still in it.

<!--BLOCK:template_substitution-->

Two of the three renderers finish, and both of them serve a page with `$date`
and `$tags` still in it. Nothing is logged, nothing raises, and the page goes
out — which is exactly the failure a template system is supposed to prevent, and
the reason to prefer the renderer that refuses. `substitute` stops at the first
name it cannot find, which is noisy in development and correct in production.

`safe_substitute` is not the safe default. It is the deliberate choice for a
value that is genuinely optional; used as the default it is the same silent
failure as `str.replace`, with better syntax.

And none of the three escapes anything. A value containing `<` is placed in the
page as `<`, which is the previous chapter's problem arriving through a new
door. The reason to use a real template engine is not the substitution syntax —
that is the easy part — it is the autoescaping, which is on by default and is
the only reason these templates can be trusted with user input.

### The accept value is arithmetic, not an echo

Four client keys, hashed with the constant the protocol defines. The count is of
distinct accept values, and of keys that reproduce the documented example.

<!--BLOCK:websocket_handshake-->

The handshake is ordinary HTTP until the last two headers, and that is the point
of it: a GET with `Upgrade: websocket` and a random client key looks like any
other request to a proxy, a load balancer or a log, so nothing in the path has
to be taught a new protocol. The server answers 101 and the connection stops
being HTTP.

The accept value is the one piece of arithmetic in the exchange and it is
deliberately not an echo. The server hashes the client's key together with the
specification's GUID, so what comes back proves the server knows the protocol
rather than that it read the request. Drop the GUID and every value changes,
which is the last row.

Every accept value is the same length, because it is base64 of a twenty-byte
SHA-1 digest: 28 characters, always. That makes it cheap to test — the
documented example is a fixed pair, so a server that reproduces it is very
likely correct and one that does not is certainly wrong. It is the same shape of
test as the CSRF token earlier in the chapter: a derived value with one
known-good example.
