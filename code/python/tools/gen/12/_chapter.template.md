...
## Counted: the list on the class, and the call behind the property

Two claims in this chapter are the kind that sound like style advice until you count them: that an
attribute written in a class body belongs to the class, and that a property is not free.

### One list, or one per instance

Ten instances of a class with a list defined on the class, then ten of a class that assigns its own.
The count is of instances whose view changed when a single instance appended to the list.

<!--BLOCK:mutable_class_attribute-->

The first row is the difference. In the shared class no instance holds an items list of its own,
because the class body ran once and created one list for the whole class. Ten instances, one append,
and ten views changed. The list on the class is not a default value; it is a single object that every
instance can reach.

The second class assigns in `__init__`, so each instance gets its own list. Ten instances, one
append, and one view changed. The last row is the one to remember: under the first design the tenth
instance can see an entry it never added.

A mutable class attribute is a shared cache that nobody declared as one. The only safe class
attributes are immutable — numbers, strings, tuples, `None`. A list, dict or set written in a class
body is a bug waiting for the second instance.

### What a property costs on every read

The second claim is about `@property`. It is the chapter's tool for keeping an invariant, and it is
worth knowing what it charges for that.

A class with a plain attribute and a class with a validated property, read the same number of times
and assigned the same number of times. The count is of getter invocations, and of assignments the
guard refused.

<!--BLOCK:property_calls-->

The second row is the price. A thousand reads of the plain attribute called nothing; a thousand reads
of the property called the getter a thousand times, because a property is a descriptor and every read
goes through it. That is a real cost, and it is why a property is worth writing for a rule rather
than for tidiness.

The last two rows are what the cost buys. Five of the fifty assignments never reached the attribute,
because the setter checked the value before storing it, and every one of them raised rather than
corrupting the object. A plain attribute has no such moment: an assignment either happens or does
not, and what it stores is whatever it was handed.

So use a property when there is an invariant to keep — a floor, a range, a derived value, a rename
that must not break callers. Do not use one to make an attribute look public, because that pays the
call on every read and enforces nothing.
