...
## Counted: the JSON you hoped for, and the codes you got

Two claims in this chapter are worth counting rather than asserting: that a response document is not
the shape you assumed, and that a status code is one of four cases rather than two.

### Reading a response that is not the shape you expected

One response document, ten paths into it, walked twice: once with square brackets and once with a
walker that gives up and returns `None`. The count is of paths that raise, and of paths that hand
back a null the caller has to notice.

<!--BLOCK:json_access-->

The exceptions are the loud half, and they are not one exception. Three paths raise `KeyError` and
one raises `IndexError`, so the `except KeyError` that looks like it covers this does not: the list
index is a different failure with a different name, arriving from the same expression.

The nulls are the quiet half and the more expensive one. Two paths succeed and hand back `None` — the
key is present and its value is null, which is not the same thing as the key being absent, and the
difference does not show up here. It shows up three functions later, as a `TypeError` on a value
somebody assumed was a string.

A walker that returns `None` for every miss collapses all of it into one outcome, and that is the
trade: no crashes, and no way to tell "absent" from "present but null" from "wrong type". Use the
walker for the fields you can do without, and check the ones you cannot with an explicit
`if key not in data: raise`, so the failure lands at the boundary where the response arrived.

### Four classes of answer, not two

The second claim is about the status line, and the code is the part of the response people read
least carefully.

Seventeen status codes a client will actually meet, classified by class, by whether
`raise_for_status()` raises on them, and by whether retrying could ever help.

<!--BLOCK:status_codes-->

The class counts are the first correction. There are four classes, not two, and the two that are not
errors matter: a 3xx is a successful answer that says "look elsewhere", and 304 is a success that
carries no body at all. A client that tests `if status == 200` throws both away.

The raise row is the second correction. Twelve of the seventeen codes raise, and they are not all the
same kind of problem. Retrying the whole group is the reflex, and half of those twelve will return
the same failure every time: a malformed request, a missing credential, a name that is not there.
Retrying a 404 costs a round trip and buys nothing.

The retry rows are the number worth keeping. Only six codes are worth a second attempt — the
timeouts, the rate limit and the server errors — so the decision has to be made on the code rather
than on the fact that something went wrong. Retry on 408, 429 and 5xx with backoff; give up
immediately on everything else and report what the body said.
