...
## Counted: two solutions to one problem, and the size of a key

Two of this chapter's problems are worth counting rather than answering, because
the count is what separates the solution everyone writes from the one worth
keeping.

### Every pair against a hash set

Two hundred numbers and one target, solved by looking at every pair and by
remembering what has already been seen. The count is of operations each one
performs to find the same pair.

<!--BLOCK:two_sum_cost-->

Both rows are correct and they are the same answer, which is what makes the
comparison worth making. The nested loop is what anyone writes first because it
is the problem restated — look at every pair — and it costs 6,047 additions to
find a pair that the hash set finds in 42 lookups.

The number that matters is not the ratio but the shape. The nested loop is
quadratic: doubling the list quadruples the work, so it is fine for 200 numbers
in an exercise and useless for the million in a log file. The hash set is linear,
and the price is that it remembers every value it has seen — a trade of memory
for time the nested loop does not make.

So the habit is not "use a hash set". It is to write the count down *before*
choosing. "Look at every pair" is a statement about a loop, and it converts into
a number: pairs, additions, comparisons. A solution whose cost you can name is
one you can defend, and the two-line change between these rows is much easier to
make once the number is on the page.

### Two ways to key an anagram

Ten base words and their rotations, keyed by sorting the letters and by counting
them. The count is of groups found, and of the characters each key is made of.

<!--BLOCK:anagram_cost-->

The first thing the table does is catch a mistake in the list above it. There are
ten base words and both keys find nine groups, because `danger` and `garden` are
anagrams of each other — obvious once it is counted, and invisible when the list
is read as ten separate ideas. Two sets of rotations are one set of words.

The second thing is that both keys agree. That check has to compare the
groupings rather than the key strings: a sorted word and a tally of letters are
never equal to each other, so comparing the keys directly would report a
disagreement between two methods that group identically — a broken test that
makes a correct program look wrong.

Then the trade. The sorted key is one character per letter, so 349 characters for
the whole list; the counted key is 26 slots whatever the word is, so 1,534
characters — four times as much, for the same answer. Sorting is the more
expensive operation and the smaller key; counting is linear and the bigger one.

Which wins depends on what you are keying. For words of six or seven letters the
sort is nothing, and the smaller key is also the one a human can read in a
debugger, which is worth more than the asymptotics at this size. The habit is to
measure the thing you are actually trading: here, the same groups and a key four
times bigger.
