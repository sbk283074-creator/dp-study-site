...
## Counted: one query two ways, and one file two ways

Two claims in this chapter are the ones that cost real data when they are believed loosely: that a
value can be put into a query, and that a comma separates fields.

### The placeholder is the channel

A users table with three rows, and six names looked up in it: one ordinary and five written to break
the query. The count is of rows returned, and of inputs that produced an answer for a name that is
not in the table.

<!--BLOCK:sql_injection-->

The two tables start identical and end different, and the only difference is that one query was built
by joining strings and the other was handed the value separately. In the formatted column the input
is part of the query, so a quote in it closes the string the author opened and whatever follows is
read as SQL. The parameterised column never gives the database a chance to misread: the statement is
fixed before the value arrives, and the value can only ever be a value.

Every one of the six inputs returned rows under the first query, and twelve rows came back in total
from a table of three. Under the second, only the one real name matched. The last two inputs matter
most: one returned every row in the table, and the other returned a row that does not exist — a
fabricated role handed to the caller as though the database had said it.

The rule has no exceptions worth making. SQL is code and values are data; they travel on different
channels, and the placeholder is the channel. This is not about escaping quotes — escaping is a
blocklist of the characters somebody thought of, and the next encoding finds another one.

### A comma is not a delimiter

The second claim is about CSV, which looks like the simplest format in the chapter and is not.

Six rows written two ways: joined with commas by hand, and written by the `csv` module. Both are read
back. The count is of fields that survive the round trip.

<!--BLOCK:csv_roundtrip-->

The `csv` module costs characters and returns rows. It quotes a field that contains a comma, doubles
a quote inside a quoted field, and keeps a newline inside the quotes where it belongs — which is why
the file it writes is longer than the one you would type, and why every row comes back.

The hand-rolled writer loses three fields and a whole row, and it loses them quietly. The row with a
comma in it has the wrong number of parts, so it is dropped; the row with a newline in it is read as
two lines, one of which looks like a perfectly good row with a truncated value. Nothing raises. The
file parses, the program finishes, and the report is wrong.

The same argument as the placeholder applies: a format with quoting rules is not a format you can
implement with `split`. Use `csv` for reading and writing, set the dialect explicitly when the
consumer is not Python, and never round-trip a value through string concatenation when a library
