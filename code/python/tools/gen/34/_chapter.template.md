...
## Counted: the tiles in one frame, and the world a seed guarantees

Two claims decide whether the world can grow: that the frame cost is set by the
viewport rather than the world, and that a seed reproduces the map exactly.

### Culling is a bound, not a measurement

A 64 by 64 world and a 20 by 15 tile viewport. The count is of tiles drawn with
and without the edge tiles, and of the tiles a boundary mistake leaves off the
screen.

<!--BLOCK:tile_culling-->

Culling is not an optimisation you add later; it is the difference between a
frame that costs 4,096 tile draws and one that costs 336. The world grows with
the game and the viewport does not, so the number to keep in the frame is the
small one — and it is small because the camera says where to stop, not because
anything was measured.

The row that matters counts the tiles the view shows but does not draw. Computing
the range as the viewport's width in whole tiles leaves the partial tile at the
right and bottom edges undrawn, so there is a strip of background along two sides
of the screen whenever the camera is not exactly tile-aligned. One extra tile in
each direction fixes it: 36 tiles out of 336, which is an extra column of 16 and
an extra row of 20.

That is the shape of an off-by-one in a loop bound. The mistake is free, the
symptom is cosmetic, and the fix is to include the boundary rather than exclude
it. The count is what makes it visible: 300 against 336 is a strip of missing
floor, not a rounding detail.

### A seed is the whole world

A 32 by 32 world generated from three seeds. The count is of tiles that differ
between runs, and of the landmasses a map is broken into.

<!--BLOCK:seeded_generation-->

The first two rows are identical tile for tile, which is the whole contract: a
seed is not a hint, it is the entire world. Two players who share a seed share a
map, and a bug report of the form "seed 4815162342, third room, the door is in
the wall" is reproducible by anyone.

The bottom rows are the part a screenshot cannot tell you. All four runs produce
a map that is roughly 58% land and they are not the same map at all — seed 1
differs from seed 2 on 474 of the 1,024 tiles. What the land count cannot see is
that seed 1 is broken into 40 separate landmasses and seed 2 into 23, so a player
who spawns in the wrong one can walk across a fraction of the world and no
further. Counting the landmasses is how a generator gets tested: not "does it
look right" but "can the player cross it".

That is why the generator has to be yours. `random` is reproducible on one
interpreter version and is not a specification; a dozen lines of arithmetic is.
When the map is part of the save file, the generator is part of the save format,
and a save format that depends on the standard library's implementation can break
in a patch release.
