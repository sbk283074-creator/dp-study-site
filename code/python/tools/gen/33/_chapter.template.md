...
## Counted: what the top scene allows, and who an event reaches

Two claims decide whether the architecture holds up as the game grows: what a
scene stack actually stops, and whether a subscriber can lose an event it was
entitled to.

### A stack decides by blocking

Five configurations of one stack. The count is of scenes that update and scenes
that draw, and it changes with two flags rather than one.

<!--BLOCK:scene_stack-->

The two pause rows are the same scene with one flag changed and they differ in
exactly the way the flag says. An opaque pause stops the play scene drawing; a
see-through one does not, so the world stays visible behind the menu while the
game is still paused. In both cases the play scene stops updating, because that
is what pausing means.

The `hud` row is why there are two flags rather than one. A heads-up display
that blocks nothing leaves the scene below it updating *and* drawing, so the
stack is an overlay rather than a mode change — two scenes running, not one.
With a single opaque flag those two cases cannot be told apart, and a status bar
would silently pause the game underneath it.

And the number that does not move: with three scenes on the stack, exactly one
updates. A stack does not mean run everything; it means the top scene decides,
and the scenes below are there to be drawn or resumed. That is also why the pop
needs a floor — the stack refuses to pop its last scene, three times out of
three, because a frame with no owner has nothing to update and nothing to draw.

### An unrelated subscriber loses the event

Five subscribers on one bus, one of which leaves during the dispatch and one of
which raises. The count is of subscribers each configuration delivers to.

<!--BLOCK:event_bus-->

The plain row delivers to four of the five and the subscriber it misses is not
the one that left. The leaver hears the event — it is removed after it has been
called, which is what "this one and then stop" means — and the subscriber that
is skipped is the one that did nothing at all. Removing an item from a list
while a loop is walking it advances the iterator past the next item, and the
victim is whoever happened to be sitting there.

The second row is what happens when you fix that. Taking a copy before iterating
removes the skip, so the raising subscriber is finally reached — and the
dispatch ends there, and the logger after it hears nothing. Delivered falls from
four to three. Fixing the silent bug exposed the loud one, which is the usual
order these are found in.

The last two rows deliver the same four and they get there for different
reasons. Taking a copy means a subscriber can leave without disturbing anyone
else; catching each exception means one broken listener cannot silence the rest.
Neither fixes the other, and a bus with only one of them still has a way for an
unrelated subscriber to lose an event. That is the price of decoupling: the
sender does not know who is listening, so the bus must not let the listeners
