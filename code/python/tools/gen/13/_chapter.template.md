...
## Counted: the order `super()` walks, and what the decorator writes

Two claims in this chapter decide how a hierarchy behaves, and both are easier to trust once they are
counted: that `super()` is not "my parent", and that a dataclass is a value type only when you ask
for one.

### The resolution order is a list

A diamond: one base, two classes inheriting it, and one class inheriting both. The count is of
classes consulted before a name is found, and of `__init__` bodies that run for a single
instantiation.

<!--BLOCK:mro_diamond-->

The third row is the part that surprises people. Three classes are consulted before `tag` is found,
because the order is a single list and the answer sits at the end of it. It is not "ask each parent,
then each grandparent" — the base appears once, in the position the algorithm gives it, and that is
what makes a diamond safe.

The last two rows are the bug. `super()` means "the next class in the resolution order of the object
being built", not "my parent", which is why the second parent runs even though the class inherits
from the first one first. Drop the call and one of the four bodies runs, so every class above the
first is never initialised — silently, with no error, on an object that looks perfectly fine.

That is the rule for anything that will ever be inherited from more than once: every `__init__` calls
`super().__init__()`, and no `__init__` assumes it is the only one running.

### What `@dataclass` writes

The second claim is about the decorator the chapter reaches for. It is described as a way to avoid
writing boilerplate, which undersells what it does and hides what it does not.

Three classes over the same two fields: one written by hand, one decorated, one frozen. The count is
of the four methods that make a value type, and of how many of the three can be used as a dictionary
key.

<!--BLOCK:dataclass_writes-->

The written column is the decorator's job. The hand-written class defines one of the four methods;
`@dataclass` writes three; a frozen dataclass writes four. None of that is a saving in typing — it is
a saving in attention, because a hand-written value class usually has `__init__` and nothing else,
and then two objects holding the same fields are not equal.

The last column is the trap. `@dataclass` sets `__hash__` to `None`, so an instance cannot be a
dictionary key or a set member: the class defines three of the four methods and is still unusable in
the two places a value type is most useful. Adding `frozen=True` writes the fourth method back.

So the default for a value object should be frozen. It buys hashing, which buys keys and sets, and it
makes the object safe to hand to a caller who might otherwise mutate it. Make it mutable only when
something has to change after construction, and accept that it then cannot be a key.
