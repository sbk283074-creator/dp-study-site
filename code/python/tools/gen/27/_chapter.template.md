...
## Counted: what a dependency costs, and where a failure lands

Two claims decide whether the layered app in this chapter behaves the way its
diagram says: how many times a dependency is built per request, and what the
client is told when the service raises.

### One construction per request

Four dependencies in a chain — settings, database, repository, service — asked
for by two endpoints. The count is of times each is constructed.

<!--BLOCK:dependency_graph-->

The left column is the default and not an optimisation you switch on. A
dependency declared with `Depends` is built once per request however many places
ask for it, so the chain costs four constructions rather than eight. That is the
behaviour, and it is the reason two handlers can both depend on the current user
and receive the same object.

The right column is what happens without it, and the shape of the number is the
point: every dependency is built twice, once for each endpoint that asked.
Settings are parsed twice, the database handle is opened twice, the repository
is created twice. On one request that is four wasted objects; across a service
it is the difference between a connection pool and a connection per call.

Which gives the rule about what a dependency may hold. The cache is scoped to a
single request, so a dependency is the right place for the current user, the
request id or a parsed body, and the wrong place for anything you want shared
across requests. A connection pool belongs to the process, created at startup
and closed at shutdown; a per-request dependency that opens a connection is a
pool you did not build.

### Failures that arrive as 500

Eight failures a service can raise, five with a handler registered. The count is
of failures that reach the client as something other than what the service meant.

<!--BLOCK:error_mapping-->

A 500 is the correct default and an uninformative answer. The three failures
without a handler are not crashes — the service knows exactly what happened —
but the client is told only that something went wrong on our side, which is true
and useless. A timeout becomes a 500 instead of a 504, a write that broke a
constraint becomes a 500 instead of a 409, and a database that is down becomes a
500 instead of a 503.

That matters because the code is the part a client acts on. 504 and 503 say the
request may work later, 409 says it will not until something changes, and 500
says only that the caller cannot tell. A client that retries on 5xx will retry
all three, including the one that will never succeed.

So the mapping is part of the API and it belongs in one place: an exception
handler per class, registered at startup, so the translation is visible rather
than scattered through the endpoints. The number to watch is the last row — how
many failures arrive as something other than what the service meant.
