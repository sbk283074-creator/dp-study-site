...
## Counted: the check that vanishes, and the test that depends on order

Two claims in this chapter are the kind that sound like warnings until you count them: that `assert`
is not a check, and that a test can pass or fail depending on what ran before it.

### The assertion that is not there

One program, five assertions, two of them deliberately false, run twice — once normally and once
with the optimiser flag. The count is of assertions that fired.

<!--BLOCK:assert_under_dash_o-->

Without the flag the run is correct: three claims hold, two do not, and the two that do not are
reported. With `-O` all five pass, and the two that are wrong are still wrong — nothing changed
except whether the check ran.

The third column is the reason. `__debug__` is true by default and false under `-O`, and `assert` is
defined as a statement that does nothing when `__debug__` is false. So an assertion is a claim that
is checked in a development run and skipped in a production one, which makes it the right tool for
the assumptions a program makes about itself and the wrong tool for validating anything a caller can
control. A validation that disappears when somebody sets a flag is not a validation; it is a comment
that costs a comparison in development and nothing in production.

### The test that depends on what ran before it

The second claim is the one every test suite eventually proves: that a test can be green for reasons
that have nothing to do with the code.

Five tests over one piece of module-level state, run in three orders. The count is of failures.

<!--BLOCK:test_order_dependence-->

The same five tests fail once in two of the orders and twice in the third, and the shared state is
the reason. `uses` passes when `loads` ran before it and fails when it did not, so the test is not
testing the code — it is testing the order.

The failure count is the useful part. A suite whose result depends on the order is a suite you
cannot trust in either direction: the green run does not mean the code is right, and the red run
does not mean it is broken. Both are reporting on what ran before.

The fix is not to sort the tests. It is to give each one its own state, so that the count is the
same in every order — and the way to know you have done it is to run the suite in a shuffled order
and check that the number does not move.
