...
## Counted: a fixed step, and a broad phase that cannot miss

Two claims decide whether the game loop is reproducible and whether its
collision detection is correct: that the update is driven by elapsed time, and
that the grid finds every collision the slow method finds.

### The update count does not move

Two seconds of loop with a fixed step of sixteen milliseconds, run at three
frame rates. The count is of update steps performed and frames drawn.

<!--BLOCK:frame_budget-->

The row that does not move is the point. Two seconds of loop performs 125 update
steps whether the machine draws 60 frames or 288, because the update is driven
by elapsed time and a fixed step rather than by the frame. That is what makes
the simulation reproducible: the same input sequence produces the same positions
on a laptop and on a desktop, and a replay is a list of inputs rather than a
recording of a session.

The cost is visible in the last rows and it is paid at the slow end. At 30 fps
each frame runs 2.08 updates, so drawing and simulation are competing for the
same frame. At 144 fps each frame runs 0.43 of an update, so most frames draw a
world that has not changed since the last one. Neither is wrong — they are the
two ends of the same trade, and the fixed step is what lets you choose where to
sit.

What it does not do is remove the need for a cap. A loop that runs free renders
as fast as the machine allows, which is 288 frames of work for 125 frames of
state, and it will spin a laptop fan to do it. The cap is not there to make the
game smooth; it is there to stop the loop spending a resource that buys nothing.

### The grid that misses

Two hundred sprites on a field, checked three ways: every pair, the same cell
only, and the full neighbourhood. The count is of pairs examined and of
collisions found.

<!--BLOCK:collision_checks-->

Two of the three rows agree and the middle one does not. Every pair is 19,900
distance checks and the full neighbourhood finds the same 468 collisions in
3,496 of them. That is the part that has to be checked rather than assumed: a
broad phase is only a speed-up if it is a speed-up that cannot miss.

The middle row is the mistake that looks like a fix, and this layout is crowded
enough to show it. A grid that compares sprites inside the same cell only
examines the fewest pairs of all — 420 — and misses 164 of the 468 collisions.
Two sprites eight pixels apart, which is exactly touching at this radius, can
sit on either side of a cell edge, and the cell that holds one never looks at
the other.

So the rule is the one the counting gives. A broad phase must be tested against
the brute-force answer on a real layout before it is trusted, and the test is
the one above: the same collisions, in fewer pairs. Everything else about it —
the cell size, the neighbourhood, the data structure — is an optimisation, and
an optimisation that changes the answer is a bug with a benchmark attached.
