...
## Counted: crossing the target, and what a heuristic promises

Two claims decide whether the AI is fair and whether its search is correct: that
reaching the target is not the same as stopping at it, and that a heuristic is a
lower bound rather than a guess.

### Crossing is not arriving

One enemy, one target, two steering behaviours. The count is of steps taken, of
steps spent near the target, and of how far past it the enemy went.

<!--BLOCK:steering_arrive-->

Both behaviours cross the target and neither stops there, which is the whole
problem. Steering is a force, and a force that always points at the target is
pointing at it from the other side one step later. The enemy does not arrive; it
orbits.

What separates the two rows is the decision to slow down. `arrive` scales the
desired speed by how close it is, so the last few units are crossed at a speed
the enemy can shed, and it settles 31 steps in. `seek` reaches the target sooner
— 22 steps against 29 — and is still moving 200 steps later, having spent 10
units past the target and never coming back.

The last two columns are the ones to tune against. Overshoot is what the player
sees as an enemy running past them and final error is what the code sees as a
failure to reach. A behaviour that is correct on average and oscillating at the
end is the most common bug in a steering system, and it is invisible in a
screenshot — which is why the arrival radius is a number in the code rather than
a feeling in the design document.

### Admissibility is a proof, not a test

A 20 by 20 grid with a wall across it, searched three ways. The count is of cells
expanded, of the path length returned, and of the cells where the estimate of the
remaining cost is larger than the truth.

<!--BLOCK:pathfinding_nodes-->

The first two rows are the same answer and they are not the same search. Without
a heuristic the queue spreads out in every direction and expands 313 cells before
it happens to reach the goal. Manhattan distance leans the queue toward the goal,
so it expands 250 and returns a path of exactly the same length.

The third row is the trap, and the column that catches it is the last one. Three
times Manhattan expands only 39 cells — by far the fastest search in the table —
and its estimate is larger than the truth on 384 of the 385 reachable cells. On
most of the map it is not a lower bound on anything; it is a guess that happens
to point the right way.

And on this map it still returns a path of 38, which is what makes admissibility
a proof rather than a test. A search with an inflated heuristic is allowed to
return a worse path, not required to; whether it does depends on the map. So a
heuristic is admissible because it can be *shown* to be, and Manhattan distance
is: no route is shorter than the straight-line steps it counts, on a grid where
every step costs one. Scaling it by anything destroys that argument, and the last
column is how much of the map the argument covered.
