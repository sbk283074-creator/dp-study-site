...
## Counted: a plan against its own claim, and the work against the file

The last two counts in the book are about the two things a reader leaves with: a
plan for the next month, and a way of reading code that is not their own.

### A plan that does not reconcile

Thirty days of work, written down the way a plan gets written down. The count is
of days filled, of days carrying more than one topic, and of the gap between what
the plan claims and what it lists.

<!--BLOCK:plan_reconciles-->

The list is fine and the sentence above it is wrong, which is the usual way a
plan fails. Thirty days are filled, so the day count reconciles; thirty-four
topics are listed, so the topic count does not. Four days carry two entries —
days 6, 13, 20 and 27 — and the sentence was written before they were.

None of this is a problem with the plan. It is a problem with reading the plan's
summary instead of the plan. A number at the top of a document is a claim about
the document, and it stops being true the moment the document changes, silently,
because nothing connects the two.

Which is why the useful habit at the end of a book is the same one it started
with. Count the thing rather than repeating what was said about it: days filled
against days available, topics listed against topics claimed, entries against
slots. The plan above is a good thirty days of work. It is just not the thirty
days it says it is, and the difference is four topics that ten seconds of
counting would have found.

### Three lines of transformation
...
<!--BLOCK:read_the_algorithm-->

The bar chart answers the question "where do I start reading this". Three lines
out of 44 are the transformation — split on a tag, take the middle, collect the
rows — and the other 41 decide whether those three lines get to run: two guards,
a retry loop, a backoff, four log calls, and the imports and constants that make
them work.

That ratio is not an accident of the example; it is what production code looks
like. The transformation is the part that differs in every file, and the guards,
the retries, the logging and the error paths are the part that is the same in all
of them. A reader who tries to understand every line in order therefore spends
most of their time on code they have seen before, and arrives at the interesting
part tired.

So read for the shape first. Find the function, read its signature and what it
returns, then find the lines that produce that return — here they are the last
three, in a helper called from the middle of a retry loop. Everything between is
usually one of five things: a guard, a retry, a log, a conversion, or a call to
something else. Knowing which five is the whole skill, and the count above is the
