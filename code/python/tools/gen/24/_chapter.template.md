...
## Counted: the parts of a URL, and the parts of a message

Two claims in this chapter are the ones the rest of the web track rests on: that a URL has parts you
can take apart, and that an HTTP message is a line, some headers, a blank line and a body.

### What the text of a URL does not say

Eight URLs a client meets, split into their components. The count is of the parts the text does not
contain and the scheme fills in, and of the URLs that are not absolute at all.

<!--BLOCK:url_parts-->

The `how` column is the first thing the text does not tell you. Three of these URLs contain no port,
and the request still goes to one — 443 for `https` and 80 for `http`, chosen by the scheme. So
`https://example.com` and `https://example.com:443` are the same request written two ways, and a
check that compares the strings rather than the parts will call them different.

The userinfo column is the one to slow down on. A URL of the form `https://user@example.com/path`
sends the request to `example.com`; everything before the `@` is a credential, not a destination.
That is the whole mechanism of a link that reads as one host and resolves to another, and it is why
the host is the part after the last `@` rather than the part after the slashes.

And one URL here is not absolute at all. A path with no scheme and no host is resolved against a
base, so `/relative/path` means something different in every context it appears in — which is fine
for an `href` inside a page and useless to a client with nothing to resolve it against.

### A line, some headers, a blank line, a body

The second claim is the shape of the message itself, which is worth reading once at the byte level
because every framework hides it afterwards.

One request and one response, built from their parts and then taken apart again. The count is of the
headers, the bytes in the body, and whether the declared length matches what arrived.

<!--BLOCK:raw_http-->

The blank line is the structure. Everything before it is lines of text a person can read; everything
after it is bytes with no shape at all, and the only thing that says where they end is
`Content-Length`. That is why a body is not text: it can be an image, a zip file, or a string that
happens to contain a blank line, and the receiver counts bytes rather than looking for a marker.

Which makes `Content-Length` a promise, and one the receiver can check, which is what the last row
does. Both messages here keep it, and the two ways of breaking it fail differently. A sender that
declares more bytes than it sends leaves the receiver waiting for bytes that never arrive, so the
download stalls and then times out. A sender that declares fewer leaves the surplus in the buffer,
where the next message will be read from it. The first looks like a slow network and the second looks
like corruption, and neither is a parsing mistake.

The status line is the other half. `4xx` is the class, and it is decided before the body is read: a
failure carries a body, and the body here is JSON, because the error is data too. A client that
checks the code and throws the body away has thrown away the only explanation it was given.
