...
## Counted: which handler runs, and how many rules a body breaks

Two claims in this chapter decide whether the API is predictable: that a path is
matched by the route you meant, and that a rejected body tells the client
everything that is wrong with it.

### The order of the routing table

Eight patterns and seven requests, each matched against the table in the order
the routes are declared. The count is of requests whose intended handler is not
the one that runs.

<!--BLOCK:path_matching-->

Two requests match more than one pattern and both are the case the chapter
warns about. `/items/new` matches `/items/new` and also `/items/{item_id}`,
because `new` is a perfectly good item id as far as the matcher is concerned;
`/users/me` matches `/users/{user_id}` in the same way.

Nothing here is ambiguous to the framework. It takes the first registered
pattern that matches and moves on, so **the order in the file is the routing
table**, and a literal path has to be registered before the parameterised path
it is a special case of. The failure is quiet in the way that costs the most
time: `/items/new` reaches the item lookup and returns a 404, so it reads as
missing data rather than as a routing mistake.

That is also why a route table is worth reading as a list rather than trusting
the decorators to sit in the right order. Once an app has forty path operations
across six files, the order is no longer something you can see, and the special
cases have to be grouped where they are declared.

### Rules that fire

Six bodies posted to one endpoint, against four rules. The count is of bodies
rejected, and of rules that reject at least one of them.

<!--BLOCK:request_validation-->

The third column is the part a hand-written `if` chain hides. The body with no
title breaks three of the four rules and the body with a numeric title breaks
two, but each is rejected by whichever rule the author happened to check first —
so the message the client receives depends on the order of the checks rather
than on what is wrong.

That is the argument for a declarative model over a chain of `if`s. A framework
validates every field and reports every failure, so a client fixing one problem
does not discover the next one on the following request. The status code follows
from the kind of rule: the body is the right shape and the values are wrong is
422, the body is not the shape at all is 400 — a distinction worth keeping,
because the client can act on one and not the other.

And the last row is the check on the rules themselves. A rule that no body ever
breaks is a rule nobody has tested, and one that every body breaks is a rule
that is wrong. All four of these fire, which is what a table of test bodies is
for.
