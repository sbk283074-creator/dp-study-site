...
## Counted: the exit code, and the run that changes nothing

Two claims in this chapter separate a script from a tool, and both are countable: that the exit code
is the part a scheduler reads, and that a job can be made safe to run twice.

### The exit code is the report

Five runs of the same job: one that worked, one that found nothing to do, and three that failed in
different ways. Each is reported by a tool that always exits 0 and by one that says what happened.

<!--BLOCK:exit_codes-->

A tool that always exits 0 is not lying about the work, it is lying about the outcome. That column
holds the same number on every row, so whatever the job did, the caller sees one value. Under cron,
or in a shell chain, or as a step in CI, that value is the whole report — the job is not read, it is
trusted.

The honest column carries four distinct codes, and the shape of them is the point. Zero means it
worked. A small number means the caller asked for something impossible, which no amount of retrying
fixes. 75 means the job could not run and should be tried again later — that is the one that
separates a transient failure from a permanent one, and it is the difference between a retry loop
that helps and one that hammers a broken endpoint forever.

So write the exit code first and the message second. The message is for the person who runs it by
hand; the code is for everything that runs it without one. And in a chain, `set -e` or `&&` is what
makes the code load-bearing — a step that fails must stop the steps after it, or the code is
decoration.

### The job you can run twice

The second claim is idempotency, which is the property that decides whether a scheduled job can be
re-run after a crash.

A directory of eight text files and a job that puts a header line on each one, run four times with
and without `--dry-run`. The count is of files changed, against the number the plan said would
change.

<!--BLOCK:dry_run_idempotency-->

The first row is the flag doing its job. Eight files need work, the plan says eight, and nothing is
written, so the person running it can read the list before anything moves. That only means something
if the dry run and the real run ask the same question, which is why both call `plan()` here. A flag
that reimplements the logic is a second program, and it will disagree with the first one eventually.

The next rows are idempotency, and they are the reason a job like this can be run from cron at all.
The second pass finds nothing to do and changes nothing, so running the job twice is the same as
running it once. Without that property a scheduled job is a job you cannot re-run after a crash: you
do not know which half already happened, and doing it again makes it worse.

The way to get it is to make the job ask "is this already done?" rather than "have I run yet?" A
marker file, a header line, a row in a table — something the job can read back and compare. State you
have to remember is state you will lose; state you can observe is state you can resume from.
