...
## Counted: the hot function, and what a save file keeps

Two claims decide whether the polish work is aimed at the right thing and whether
the save format will still load in a year: which function is actually expensive,
and which parts of the state JSON can hold.

### The busiest function is not the expensive one

One frame of a game with four instrumented functions. The count is of calls, of
the work each call does, and of the total.

<!--BLOCK:profiling_by_count-->

The two answers disagree, and that is the lesson. Counting calls says `draw_text`
is hot: it is called 240 times against `check_collisions`'s one. Counting work
says `check_collisions`, which is called once and does 19,900 operations — 67% of
the frame on its own, against 24% for every text draw put together.

A call counter is the cheapest profiler there is and it is almost never the one
you want. Functions called in a loop are cheap by construction, because a loop
that calls something expensive per iteration is a loop nobody writes twice. The
expensive calls are the ones made once, at the top of the frame, over the whole
world.

So the number to sort by is the product of the two columns, and the way to get it
is to count operations rather than seconds. An operation count is the same on
every machine, it can be compared between two versions of the code in a test, and
it does not change when the laptop is warm — which is why it can be printed in a
chapter like this one.

### Three entries JSON cannot keep

A game state of ten entries saved through `json`. The count is of entries whose
type survives the round trip, of entries whose type changes, and of entries that
cannot be written at all.

<!--BLOCK:save_compatibility-->

Seven of the ten come back as what they were, which is why JSON is the right
default and why it is easy to stop thinking about. The other three are the whole
subject of a save format. The position was a tuple and is now a list — equal to
nothing that compares types, and the kind of bug that arrives as a coordinate
that will not index. The set of visited rooms and the player object are not JSON
at all: one has no object notation, and the other has no way to know which class
to rebuild.

None of that is a defect in the library. JSON has six types and a game has more,
so a save format is a decision about how to represent the difference — and the
decision has to be written down, because a save file outlives the code that wrote
it. A player who comes back in two years is loading a file your program no longer
exists to explain.

So the shape of a good save format is a version number, a plain dictionary of
JSON-safe values, and a loader that knows the schema. The version is what lets
the loader do something other than crash when it meets a file from an older
build, and the schema is what turns the list back into a tuple and the name back
into a `Player`. Pickle does all of that in one line and takes the decision away
from you, which is the trade: convenient until the class moves, and unreadable to
anyone who wants to fix a save by hand.
