...
def load_keys(pkg):
    """Read a keys.py by explicit path.

    Both question banks ship a file called keys.py, so a plain `import keys` would
    silently shadow one with the other depending on sys.path order. Loading by path
    keeps the two maps separate and makes the collision impossible.
    """
    path = os.path.join(TOOLS, pkg, "keys.py")
    spec = importlib.util.spec_from_file_location("keys_" + pkg, path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod.KEYS


# Two banks, one teaching layer. The 25 past-paper questions and the 12 sample
# questions share every lesson, so the reverse index has to be built from the union:
# a concept reached only by the sample sheet must still resolve, or the
# "every concept is used by at least one question" gate fires on a real lesson.
PAPER_KEYS = load_keys("paper2025")
SAMPLE_KEYS = load_keys("r0sample")
KEYS = dict(PAPER_KEYS)
for _qid, _ks in SAMPLE_KEYS.items():
    if _qid in KEYS:
        raise SystemExit("question id %s appears in both key maps" % _qid)
    KEYS[_qid] = _ks

OUT = os.path.normpath(os.path.join(HERE, '..', '..', 'data', 'concepts.js'))

CONCEPTS = concepts_a.C + concepts_b.C + concepts_c.C + concepts_d.C
errors = []
...
lines.append("")
lines.append("   Generated by tools/paper2025/build_concepts.py from concepts_a/b/c/d.py. Edit those,")
...
lines.append("   index: which of the 37 questions use this point -- the 25 from the 2025 paper plus")
lines.append("   the 12 sample questions.")
lines.append("")
lines.append("   %d key points, used %d times across the two banks." % (len(CONCEPTS), sum(len(v) for v in used_by.values())))
lines.append("*/")
