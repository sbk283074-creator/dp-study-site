# -*- coding: utf-8 -*-
"""Figures for BANK SECTION 01.  Run:  python figs.py

Every figure is hand-authored inline SVG, emitted into fig/.  There are no image
files and nothing is fetched, because the site has to work over file://.

Three rules, each of which fails silently if broken (see the project notes):

  1. A figure-bearing `q` in sec01.py must be a BACKTICK template string, because
     the SVG attributes use double quotes.  The build step handles that.
  2. Marker ids must be unique across the WHOLE section, because HTML has no id
     namespace: two figures both defining id="ar" collide and one of them silently
     gets the other's arrowhead.  Every marker here is prefixed with the figure key.
...
The house rule for the drawings themselves: a figure must encode the discriminator
the question turns on, not decorate the apparatus.  So the lamp characteristic bends
over (that is what makes the gradient-versus-ratio distinction bite), the bouncing
ball panels differ in their peak heights (that is the energy loss), and the fibre
shows both boundaries (the acceptance angle depends on both).
"""
import math
import os
...
def main():
    ...
    stale = sorted(f[:-4] for f in os.listdir(OUT)
                   if f.endswith('.svg') and f[:-4] not in FIGS)
    if stale:
