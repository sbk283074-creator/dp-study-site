...
# ═════════════════════════════════════════════════════════════════════════════
# 12 — H, circuits.  DEEP (10 moves).  Removing a branch, and the terminal p.d.
# asked for as a RATIO rather than a value.
# ═════════════════════════════════════════════════════════════════════════════
{
    "n": 12, "id": "S01-12", "module": "H", "diff": 3,
    "topic": "Removing one branch of the load: the ratio of the two terminal potential differences",
    "rel": [("H", "A parallel pair reduces to a single resistance before the loop can be treated as a series loop"),
            ("H", "Ohm's law applied to the whole circuit, internal resistance included"),
            ("H", "The terminal potential difference is the emf minus the volts lost across the internal resistance"),
            ("A", "Answering a question about a ratio by dividing two expressions instead of evaluating either one")],
    "key": ["terminal", "internal", "ratio", "parallel", "load"],
    "stem": '<p>The figure shows a cell of emf 12 V and internal resistance 1.0 &#937; driving a load made of a 6.0 &#937; resistor in series with a parallel pair of 6.0 &#937; and 3.0 &#937;. {{FIG:s01-12}}</p><p>The 3.0 &#937; resistor is now removed, so the cell drives the two 6.0 &#937; resistors in series. Nothing else is changed.</p><p>What is the ratio <code>(terminal p.d. after the removal) / (terminal p.d. before the removal)</code>?</p>',
    "opts": ['27/26', '9/13', '3/2', '1', '26/27'],
    "ans": 0,
    "distractors": ['correct',
                    'answers the wrong quantity: it is the ratio of the two LOST voltages, <code>I r</code>, and not of the two terminal p.d.s',
                    'is the ratio of the two external resistances, 12.0/8.0, which would be right only if the internal resistance were zero',
                    'assumes the terminal p.d. does not depend on the load at all, which is the misconception the question is built on',
                    'inverts the correct ratio, so the terminal p.d. is taken to fall when a branch is removed'],
    "profile": {
        "steps": [
            ("relate", "reduce the parallel pair to one resistance, since the loop cannot be treated as a series loop until it is"),
            ("relate", "add the series resistor to get the external resistance before the change"),
            ("solve", "add the internal resistance and divide the emf by the total to get the current before the change"),
            ("solve", "multiply that current by the external resistance to get the terminal p.d. before the change"),
            ("relate", "note that removing one branch of a parallel pair leaves the other branch alone, so the new external resistance is a plain series sum"),
            ("solve", "repeat the division for the new current"),
            ("solve", "repeat the multiplication for the new terminal p.d."),
            ("solve", "divide the two terminal p.d.s and cancel the common factors"),
            ("check", "get the same ratio from the short form V = E R/(R + r), which never needs either current"),
            ("check", "settle the direction of the change, which alone rejects three of the five options"),
        ],
        "relations": ["R_parallel = R1 R2 / (R1 + R2)",
                      "I = E / (R_ext + r)",
                      "V = I R_ext",
                      "V = E R_ext / (R_ext + r)",
                      "lost volts = I r"],
        "insight": "The terminal p.d. is a property of the cell and the load together, so changing the load changes it. Removing a branch raises the external resistance, which lowers the current, which lowers the volts lost inside the cell, which raises the terminal p.d. The ratio must therefore exceed 1 before any arithmetic is done.",
        "shape": "ratio-cancellation",
        "approx": False,
        "symbolic": False,
        "figure_support": True,
    },
    "check": {"kind": "eval", "expr": "((12/(6+6+1))*12)/((12/(6+(6*3)/(6+3)+1))*(6+(6*3)/(6+3)))", "want": "27/26"},
...
# ═════════════════════════════════════════════════════════════════════════════
# 16 — B, kinematics.  DEEP (10 moves).  A stone thrown UP from a cliff top: the
# full flight is one quadratic, and the answer is the expression, not a number.
# ═════════════════════════════════════════════════════════════════════════════
{
    "n": 16, "id": "S01-16", "module": "B", "diff": 3,
    "topic": "A stone thrown upward from a cliff top: the time to reach the foot as an expression",
    "rel": [("B", "Describing the whole vertical flight with a single position equation, taking upward as positive"),
            ("B", "Asking when the stone reaches a given height turns the motion into a quadratic in the time"),
            ("A", "Choosing the physically meaningful root of a quadratic and discarding the other"),
            ("C", "Conservation of energy as an independent route to the speed of impact")],
    "key": ["projectile", "cliff", "quadratic", "vertical"],
    "stem": '<p>A stone is thrown vertically upward with speed <code>u</code> from the top of a vertical cliff of height <code>h</code>. It rises, falls back past the point of release, and lands at the foot of the cliff. The acceleration due to gravity is <code>g</code>. {{FIG:s01-16}}</p><p>Which expression gives the time from release to landing?</p>',
    "opts": ['(u + &#8730;(u<sup>2</sup> + 2gh)) / g',
             '&#8730;(u<sup>2</sup> + 2gh) / g',
             'u/g + &#8730;(2h/g)',
             '2u/g + &#8730;(2h/g)',
             '&#8730;(2h/g)'],
    "ans": 0,
    "distractors": ['correct',
                    'keeps only the descending part of the flight, so the time spent rising is left out of the total',
                    'adds the time to rise to the time a stone would take to fall the height of the cliff from rest, as though the two parts did not join up',
                    'adds the whole up-and-back-down time to that same free-fall time, counting the rise twice',
                    'ignores the throw altogether and treats the stone as dropped from rest at the cliff top'],
    "profile": {
        "steps": [
            ("relate", "fix a sign convention and write one position equation that covers the whole flight, including the part above the release point"),
            ("solve", "differentiate, or read off, the time at which the velocity is zero to get the time to the highest point"),
            ("solve", "substitute that time back to get the greatest height above the release point"),
            ("relate", "write the landing condition as a height rather than a distance, since the stone finishes below where it started"),
            ("solve", "rearrange into the standard quadratic form and identify the three coefficients"),
            ("solve", "apply the quadratic formula and keep the positive root, discarding the negative one as an unphysical time before release"),
            ("check", "simplify the surviving root and confirm that it is larger than the time to the highest point"),
            ("relate", "get the speed of impact from the velocity equation and note that it is negative, so the stone is moving downward"),
            ("check", "get the same speed from energy, which never needs the time and so is a genuinely independent route"),
            ("check", "test the two limits h &#8594; 0 and u &#8594; 0, each of which has an answer known by inspection"),
        ],
        "relations": ["y = u t &#8722; ½ g t<sup>2</sup>",
                      "t_top = u / g",
                      "½ g t<sup>2</sup> &#8722; u t &#8722; h = 0",
                      "t = (u + &#8730;(u<sup>2</sup> + 2gh)) / g",
                      "v<sup>2</sup> = u<sup>2</sup> + 2gh"],
        "insight": "The rise and the fall are not two separate problems that get added together. One position equation covers the entire flight, and the landing point is simply the height minus h, which makes the whole thing a single quadratic in the time.",
        "shape": "algebraic-elimination",
        "approx": False,
        "symbolic": True,
        "figure_support": True,
    },
    "check": {"kind": "eval", "expr": "sqrt(10*10 + 2*10*120) + (10 - 10*6)", "want": "0"},
