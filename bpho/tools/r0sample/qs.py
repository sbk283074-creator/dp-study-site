# -*- coding: utf-8 -*-
"""BPhO Round 0 — the 2025 SAMPLE QUESTIONS (S1..S12), with full worked solutions.

Source: "Round_0_Sample_Questions_2025.pdf", 3 pages, 12 questions, headed
"12 Sample Questions (non-calculator)". This is the sheet BPhO publishes so that
candidates can see the style before sitting the paper.

Every answer below was derived independently and then cross-checked against the
printed figure, whose geometry was recovered with pypdfium2 vector extraction --
never read off a rendered image, because this model cannot see images. Four of the
twelve turn on something that is only decidable from the drawing:

  S3  the polarity of both cells: the LONG plate is the positive terminal, and the
      stroke lengths (15.0 vs 7.5) put BOTH long plates on the left. Had they
      opposed, the link current would have been 1.5 A, not 0.50 A.
  S7  where the extra load sits: the sheet's outline path begins at (306,246),
      which is exactly the midpoint of the edge joining the two corners labelled R.
      That is what makes moments about that edge so clean.
  S8  the shape of each sketch: A is a STRAIGHT fall to the axis, C is flat then
      plunging, E is steep then flattening and never reaches the axis. Only E has
      the shape of arcsin(1/n). A wrong reading here is invisible in a spot check.
  S6  the exponent on option E: it is A^2 s^3 J^-1, NOT A^2 s^2 J^-1. If it were the
      latter it would also be a farad and the question would have two right answers.

The answer letter distribution is 6 B, 3 D, 2 E, 1 A and no C. That is the real
sheet, not an error: a 12-question sample has no obligation to be uniform, and
"fixing" it toward uniformity would falsify the record.
"""

QUESTIONS = [

{
    "id": "R0S-01", "n": 1, "module": "K", "topic": "Nuclide notation: which of these two nuclei are alike?", "diff": 1,
    "rel": [
    ...
    "stem": "<p>Two nuclei, <sup>3x</sup><sub>2x</sub>Q and <sup>4x</sup><sub>3x</sub>R "
            "(<code>x</code> is an integer), have the same:</p>",
    "fig": None,
...
{
    "id": "R0S-02", "n": 2, "module": "F", "topic": "Counting half-wavelengths on a string with five antinodes", "diff": 1,
    "rel": [
    ...
    "stem": "<p>A string has two fixed ends, separated by a distance of 10 m. A stationary wave on "
            "the string has five antinodes. What is the wavelength?</p>",
    "fig": None,
...
{
    "id": "R0S-03", "n": 3, "module": "H", "topic": "Two cells and two resistors sharing a zero-resistance link", "diff": 2,
    "rel": [
    ...
    "stem": "<p>Calculate the current labelled <code>I</code> in the circuit below.</p>{{FIG:r0s-03}}",
    "fig": None,
...
{
    "id": "R0S-04", "n": 4, "module": "G", "topic": "Fringe spacing when the whole double-slit apparatus goes under water", "diff": 1,
    "rel": [
    ...
    "stem": "<p>A double-slit diffraction experiment, using monochromatic light, is carried out in air and the "
            "fringe spacing <code>w</code> on a distant screen is measured. An identical experiment is set up "
            "under still water, which has refractive index <code>n</code>.</p>"
            "<p>What is the fringe spacing that is measured on the underwater screen?</p>",
    "fig": None,
...
{
    "id": "R0S-05", "n": 5, "module": "H", "topic": "Power in one resistor: same two resistors in series, then in parallel", "diff": 2,
    "rel": [
    ...
    "stem": "<p>A cell of internal resistance <code>r</code> is connected across two identical resistors, each of "
            "resistance <code>4r</code>, in series. The power dissipated in one of the resistors is "
            "<code>P<sub>1</sub></code>. The cell is removed and connected across the same two resistors, but this "
            "time they are in parallel. The power dissipated in one of the resistors is now "
            "<code>P<sub>2</sub></code>.</p>"
            "<p>Find <code>P<sub>1</sub> / P<sub>2</sub></code>.</p>",
    "fig": None,
...
{
    "id": "R0S-06", "n": 6, "module": "I", "topic": "Which combination of units is a farad?", "diff": 2,
    "rel": [
    ...
    "stem": "<p>Which one of the following is a unit of capacitance?</p>"
            "<p><i>[Hint: capacitance is the charge stored per unit potential difference.]</i></p>",
    "fig": None,
...
{
    "id": "R0S-07", "n": 7, "module": "C", "topic": "Reactions at three rods when a weight is added halfway along an edge", "diff": 3,
    "rel": [
    ...
    "stem": "<p>A uniform rigid metal sheet in the shape of an equilateral triangle has weight <code>W</code> and is "
            "supported on horizontal ground by three light inextensible rods of equal length at each of its "
            "corners. A weight <code>W</code> (same weight as the sheet) is placed on top of the sheet halfway "
            "between two corners, as shown below.</p>"
            "<p>Find the ratio of reaction forces <code>R : N</code> acting on the rods from the ground.</p>"
            "{{FIG:r0s-07}}",
    "fig": None,
...
{
    "id": "R0S-08", "n": 8, "module": "G", "topic": "Which sketch shows how the critical angle varies with refractive index?", "diff": 3,
    "rel": [
    ...
    "stem": "<p>Consider the boundary between a material with refractive index <code>n</code> and air. Which of "
            "the following graphs best describes how the critical angle <code>θ<sub>c</sub></code> changes with "
            "<code>n</code>?</p>{{FIG:r0s-08}}",
    "fig": None,
    "opts": ["A — falls in a straight line to the axis",
             "B — rises, steepening as it goes",
             "C — runs almost flat, then plunges to the axis",
             "D — rises, then levels off",
             "E — falls steeply at first, then flattens without reaching the axis"],
    "ans": 4,
...
{
    "id": "R0S-09", "n": 9, "module": "C", "topic": "Energy stored when three springs are displaced sideways by a small amount", "diff": 3,
    "rel": [
        ("C", "Energy stored in a stretched spring, ½kx²"),
        ("E", "Hooke's law and spring constant"),
        ("A", "The small-angle and limiting-case checks"),
    ],
    "stem": "<p>Three ideal springs of spring constant <code>k</code> and natural length <code>a</code> are arranged "
            "equidistant from a point <code>P</code>, as shown below. The points <code>A</code>, <code>B</code> and "
            "<code>C</code> are fixed in the plane of the paper, and are initially each a distance <code>a</code> "
            "away from <code>P</code>. The point <code>P</code> is then moved a small distance <code>d</code> "
            "perpendicular to the plane of the paper.</p>"
            "<p>Which of these options gives the best approximation for the total energy stored in the system?</p>"
            "{{FIG:r0s-09}}",
    "fig": None,
    "opts": ["kd<sup>4</sup> / (2a²)", "3kd<sup>4</sup> / a²", "3kd<sup>4</sup> / (2a²)",
             "3kd<sup>4</sup> / (4a²)", "3kd<sup>4</sup> / (8a²)"],
    "ans": 4,
...
{
    "id": "R0S-10", "n": 10, "module": "A", "topic": "How the ring frequency of a rod depends on its length", "diff": 2,
    "rel": [
    ...
    "stem": "<p>The frequency <code>f</code> produced by a uniform metal rod when hit by a hammer is dependent only "
            "upon its length <code>L</code>, density <code>ρ</code> and Young's modulus <code>E</code>. Find the "
            "proportionality relation between <code>f</code> and <code>L</code>.</p>",
    "fig": None,
...
<div class="formula">f = C × L⁻¹ ρ^(−1/2) E^(1/2)  =  (C / L) × √(E / ρ)</div>
...
{
    "id": "R0S-11", "n": 11, "module": "D", "topic": "Why the ground pushes less hard on a rock at the equator", "diff": 2,
    "rel": [
    ...
    "stem": "<p>A small rock sitting at the Earth's North pole experiences a normal reaction force "
            "<code>N<sub>1</sub></code> from the ground. The same rock at the equator experiences a normal "
            "reaction force <code>N<sub>2</sub></code>. The Earth's radius is <code>6 × 10⁶ m</code>.</p>"
            "<p>Which of these is the best estimate for <code>(N<sub>1</sub> − N<sub>2</sub>) / "
            "N<sub>1</sub></code>?</p>",
    "fig": None,
...
{
    "id": "R0S-12", "n": 12, "module": "C", "topic": "Tension in a chain whose blocks halve in mass going back", "diff": 3,
    "rel": [
    ...
    "stem": "<p>A long chain of blocks, connected by light inextensible strings, travels without friction in a "
            "straight line on horizontal ground. The front block is being pulled with a force <code>D</code>, "
            "causing the entire chain to accelerate at a rate <code>a</code>. The block at the front has mass "
            "<code>m</code>, and each other block has half the mass of the block directly in front of it. The "
            "tensions in the strings are labelled <code>T<sub>1</sub></code>, <code>T<sub>2</sub></code>, ... from "
            "the front, as shown in the diagram.</p>"
            "<p>Show that <code>T<sub>n</sub> = D − f(n)ma</code>, and select the correct form of the function "
            "<code>f(n)</code>.</p>{{FIG:r0s-12}}",
    "fig": None,
...
<div class="formula">block 1:  m        block 2:  m/2      block 3:  m/4      ...      block n:  m / 2^(n−1)</div>
...
<div class="formula">m/2ⁿ + m/2^(n+1) + m/2^(n+2) + ... = (m/2ⁿ)(1 + 1/2 + 1/4 + ...) = (m/2ⁿ) × 2 = m / 2^(n−1)</div>
<p>Check it against the case you can do in your head: for <code>n = 1</code> the tail is everything but the front block, whose total mass is <code>m/2 + m/4 + ... = m</code>. And <code>m/2^(1−1) = m</code>. ✓</p>
<p><b>Step 4 — get the tension.</b></p>
<div class="formula">Tₙ = (m / 2^(n−1)) × a = ma × 2^(1−n)</div>
...
<div class="formula">D − Tₙ = 2ma − ma × 2^(1−n) = ma (2 − 2^(1−n))</div>
<p>so, writing <code>Tₙ = D − f(n)ma</code>:</p>
<div class="formula">f(n) = 2 − 2^(1−n)</div>
<p><b>Answer: D, 2 − 2<sup>1−n</sup>.</b></p>
<p><b>Test the formula at n = 1 and n = 2 — this is the whole verification.</b> At <code>n = 1</code>, <code>2^(1−1) = 2⁰ = 1</code>, so <code>f(1) = 2 − 1 = 1</code> and</p>
<div class="formula">T₁ = D − ma = 2ma − ma = ma</div>
<p>which agrees with the direct calculation in step 4, <code>T₁ = ma × 2⁰ = ma</code>. ✓ At <code>n = 2</code>, <code>2^(1−2) = 1/2</code>, so <code>f(2) = 2 − 1/2 = 3/2</code> and</p>
...
<p><b>Why the other four are wrong.</b> Option C, <code>2 − 2^(−n)</code>, differs from the right answer by a single <code>1</code> in the exponent, and testing it at <code>n = 1</code> kills it: it gives <code>f(1) = 3/2</code>, hence <code>T₁ = ma/2</code>, but you can read <code>T₁ = ma</code> straight off the definition, since the string behind the front block tows a tail of total mass <code>m</code>. Option B, <code>2^(1−n)</code>, is the tension divided by <code>ma</code> rather than the shortfall <code>f(n)</code> — it is the answer to a different rearrangement. Option E, <code>1 − 2^(−n)</code>, gives <code>T₁ = 1.5 ma</code>, which is larger than <code>T₁</code> can be. Option A, <code>2 − n</code>, goes negative for <code>n &gt; 2</code>, so it would make the tension exceed the driving force.</p>
