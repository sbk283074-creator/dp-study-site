...
## 11. Found while executing §8 — a stale link this review itself missed

§1's table claimed **"zero `PYTHON/` references remain"**. That was true of the static HTML and
false of the running site. `assets/ai-widget.js` keeps its **own hardcoded site map**, and it had
three stale references:

| Line | Was | Now |
|---|---|---|
| 82 | `{ t: "Python Mastery", …, u: "PYTHON/" }` in the `SITE` array | `{ t: "Code Mastery", …, u: "code/" }` |
| 182 | path→label returned `"Python Mastery"` for any path matching `/python` | plus `if (p.indexOf("/code/") > 0) return "Code Mastery"` |
| 321 | the "suggested flow" line linked to `HUB + 'PYTHON/'` | `HUB + 'code/'`, labelled Code Mastery |

The widget builds its nav in JavaScript, so **no grep of the HTML can see these** — which is
precisely why §1's table was wrong. Verified **6/6** against the live site with the fixed widget
injected (the local run can only reach 5/6, because the widget resolves its entries against the
production hub and `/code/` therefore cannot match on `127.0.0.1`).

### The `?v=` bump: a site-wide decision, left for Lucas

The widget is loaded from the **absolute production URL** with a cache-buster:

```html
<script src="https://sbk283074-creator.github.io/dp-study-site/assets/ai-widget.js?v=13" defer></script>
```

**Measured, not assumed.** Because the file's *content* changed, a fresh browser requesting the
unchanged `?v=13` URL now receives the **fixed** widget — confirmed live, 6/6 checks passing with no
request interception. So the fix **is** reaching new visitors. The `?v=` number only matters for
browsers that already cached `v=13`; they keep the old copy until it is bumped.

Bumping it means editing **~400 files** that load the widget, including all of `challenge-bank/site/`
(generated by `challenge-bank/build.py`, which carries the version string itself) — which crosses
into the concurrent session's territory. Not done unilaterally, and **not urgent**: the stale links
still work, because `/PYTHON/` redirects to the new book. The only symptom on a cached browser is
that the widget's nav still calls the platform "Python Mastery".
