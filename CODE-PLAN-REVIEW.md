...
### Open: the `?v=` bump is a site-wide decision, left for Lucas
...
So the fix is committed but **not yet reaching readers** — browsers keep serving the cached `v=13`
build. Deploying it means bumping `?v=13` → `?v=14` in **every page that loads it: ~400 files**,
including all of `challenge-bank/site/` (generated by `challenge-bank/build.py`, which carries the
version string itself). That crosses into the concurrent session's territory, so it was not done
unilaterally.

Urgency is low: the stale links still **work**, because `/PYTHON/` redirects to the new book.
Nothing is broken — the widget's nav just still calls the platform "Python Mastery".
